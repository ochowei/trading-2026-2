"""TSM「量先換手＋價行止跌」時間衰減版短線均值回歸研究的可重算核心。

本模組只接受已凍結的日線 OHLCV，不下載資料、不連線券商，也不建立真實委託。
候選整合 20 日均線負偏離（>= 1.5%）、過去 5 個交易日成交量先行爆量（>= 1.25 倍均量）
與訊號日反轉收紅（Close > prior Close）確認，並將反彈後的 2 日 RSI 限制在 50 以下
（確保仍處於均值回歸早期非過熱區間）。本輪只改變主要策略假說：反轉優勢若在
10 個完整持有 session 內沒有實現，就提早以 time exit 退出，避免把訊號拖成過期部位。
SMA＋RSI baseline 透過關閉成交量先行與價格反轉兩個額外條件，沿用完全相同的執行規則。
"""
