"""TSM 兩階段止跌確認短線均值回歸研究的可重算核心。

本模組只接受已凍結的日線 OHLCV，不下載資料、不連線券商，也不建立真實委託。
候選先以 20 日均線負偏離至少 1.5%、RSI(2) 不高於 50，及前五個交易日內的
1.05 倍成交量確認建立資格；資格當日保留 v009 的同日收盤回升訊號，否則最多
等待後續三個交易日，第一次收盤高於前一日且仍低於當日 20 日均線時，於下一個
交易日開盤進場。每次資格最多使用一次，等待中的新資格不會重設或延長期限。
"""
...
class StrategySpec:
    ...
    delayed_confirmation: bool = True
    qualification_valid_sessions: int = 3
...
V009_CONTROL_SPEC = DEFAULT_SPEC.with_changes(delayed_confirmation=False)
...
class Trade:
    ...
    held_sessions: int
    qualification_session: pd.Timestamp
    confirmation_type: Literal["same-day", "delayed"]
...
def indicators(bars: pd.DataFrame, spec: StrategySpec = DEFAULT_SPEC) -> pd.DataFrame:
    """只用當下及更早 session 計算訊號；成交量條件只讀取過去 session。"""
    ...
    result["qualification_signal"] = qualification_signal
    same_day_signal = qualification_signal.copy()
    if spec.require_close_above_prior_close:
        same_day_signal &= result["close_above_prior_close"]
    result["same_day_confirmation_signal"] = (
        result["close_above_prior_close"] & (clean["Close"] < result["sma_20"])
    )
    result["raw_signal"] = same_day_signal
    return result
...
def backtest(
    ...
    *,
    ...
    當 ``delayed_confirmation`` 開啟時，資格日是狀態建立日，後續確認日才是
    訊號日；資格只保留到資格日後的第三個交易日收盤，且等待中的新資格會被忽略。
    """

    if (
        spec.cooldown_sessions < 0
        or spec.holding_sessions <= 0
        or spec.qualification_valid_sessions < 1
    ):
        raise ValueError("cooldown、持有期或資格有效期設定不合法")
    frame = indicators(bars, spec)
    cash = float(spec.initial_cash)
    trades: list[Trade] = []
    accepted: list[pd.Timestamp] = []
    pending_signal_index: int | None = None
    pending_signal_qualification_index: int | None = None
    pending_signal_confirmation_type: Literal["same-day", "delayed"] | None = None
    waiting_qualification_index: int | None = None
    ...
    for index, (session, bar) in enumerate(frame.iterrows()):
        if position is not None and time_exit_index == index:
            ...
            trades.append(
                Trade(
                    ...
                    qualification_session=position["qualification_session"],
        ...
        if pending_signal_index is not None and pending_signal_index + 1 == index:
            ...
            if shares > 0:
                ...
                assert pending_signal_qualification_index is not None
                assert pending_signal_confirmation_type is not None
                position = {
                    "signal_session": frame.index[pending_signal_index],
                    "entry_session": session,
                    "qualification_session": frame.index[
                        pending_signal_qualification_index
                    ],
                    "confirmation_type": pending_signal_confirmation_type,
                    "raw_entry": raw_entry,
            ...
            pending_signal_qualification_index = None
            pending_signal_confirmation_type = None
        ...
        if can_process_signal and spec.delayed_confirmation:
            waiting_was_active = waiting_qualification_index is not None
            if waiting_was_active:
                assert waiting_qualification_index is not None
                expires_at = (
                    waiting_qualification_index + spec.qualification_valid_sessions
                )
                if index > expires_at:
                    waiting_qualification_index = None
                    waiting_was_active = False
                else:
                    confirmation_detected = (
                        (
                            index == waiting_qualification_index
                            and bool(bar["raw_signal"])
                        )
                        or (
                            index > waiting_qualification_index
                            and bool(bar["same_day_confirmation_signal"])
                        )
                    )
                    if confirmation_detected:
                        qualification_index = waiting_qualification_index
                        waiting_qualification_index = None
                        waiting_was_active = True
                        if enough_time_to_exit and lifecycle_within_end:
                            accepted.append(session)
                            pending_signal_index = index
                            pending_signal_qualification_index = qualification_index
                            pending_signal_confirmation_type = (
                                "same-day"
                                if index == qualification_index
                                else "delayed"
                            )

            if (
                not waiting_was_active
                and waiting_qualification_index is None
                and enough_time_to_exit
                and lifecycle_within_end
                and bool(bar["qualification_signal"])
            ):
                waiting_qualification_index = index
                if bool(bar["raw_signal"]):
                    waiting_qualification_index = None
                    accepted.append(session)
                    pending_signal_index = index
                    pending_signal_qualification_index = index
                    pending_signal_confirmation_type = "same-day"
