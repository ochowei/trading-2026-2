"""TSM「兩階段量先價行」適度換手與動態容量提升版短線均值回歸研究的可重算核心。

本模組只接受已凍結的日線 OHLCV，不下載資料、不連線券商，也不建立真實委託。
候選整合 20 日均線負偏離（>= 1.2%）、訊號日反轉收紅（Close > prior Close）、
反彈後仍低於 55 的 2 日 RSI，以及前 5 個 session 內出現成交量溫和放大
（Volume / SMA20(Volume.shift(1)) >= 1.05）的量先特徵。
相較 v009 採用過度緊縮的 1.5% 均線偏離與 50 的 2 日 RSI 門檻導致交易樣本過少
（5 年僅 24 筆，2017 年僅 2 筆，逼近 20 筆失格邊緣），且退場後強制冷卻 5 個 session
過度壓抑交易容量，本輪（v010）將均線負偏離門檻微調為 1.2%、RSI 上限適度放寬為 55
（化解收紅日若強勁反彈便無法小於 50 的弱勢夾層限制），並將平倉冷卻期縮短至 3 個 session，
在維持優異獲利因子與穩定度的前提下，有效提升交易機會（Development 達 29 筆，2017 年達 3 筆）。
10 個完整持有 session 內未實現停損或停利的部位於下一個 open 以 time exit 退出。
baseline 關閉成交量先行與價格反轉條件，保留相同執行規則。
"""
...
class StrategySpec:
    ...
    mean_reversion_min: float = 0.012
    rsi_lookback: int = 2
