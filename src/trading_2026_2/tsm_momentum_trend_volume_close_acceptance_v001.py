"""TSM v001「量能加權收盤承接先於價格加速」研究引擎。

本模組測試一個和既有「多日量能集中」、「單次量能脈衝」、「高量低變動」及
「量價活動效率」不同的量價順序：訊號日前五個已完成 session 的成交量，是否
先集中在收盤接近日內高點的 session，形成買方收盤承接，再由訊號日的價格加速
確認動能釋放。指標比較的是成交量加權收盤位置與未加權收盤位置的差，不是只
計算總量、單一尖峰、日內振幅或高量日的收盤變動大小。
...
class StrategySpec:
    ...
    close_acceptance_window: int = 5
    close_acceptance_min_periods: int = 5
    close_acceptance_spread_min: float = 0.01
    volume_weighted_close_location_min: float = 0.48
    close_acceptance_enabled: bool = True
    # v003 legacy contract names are retained so the workflow can compare
    # common volume lookback and prior-session boundaries. Their semantics
    # here are the new close-acceptance mechanism, not the old volume-lead rule.
...
BASELINE_SPEC = DEFAULT_SPEC.with_changes(
    close_acceptance_enabled=False,
    volume_lead_enabled=False,
)
CLOSE_ACCEPTANCE_ONLY_SPEC = DEFAULT_SPEC.with_changes(
...
def _required_history_sessions(spec: StrategySpec) -> int:
    ...
    return max(
        ...
        spec.volume_average_min_periods + spec.close_acceptance_window,
...
def indicators(bars: pd.DataFrame, spec: StrategySpec = DEFAULT_SPEC) -> pd.DataFrame:
    ...
    volume_ratio = volume / prior_volume_average
    intraday_range = clean["High"] - clean["Low"]
    close_location = ((close - clean["Low"]) / intraday_range).where(
        intraday_range > 0,
        0.5,
    )
    close_location = close_location.clip(lower=0.0, upper=1.0)
    prior_window_volume = volume.shift(1).rolling(
        spec.close_acceptance_window,
        min_periods=spec.close_acceptance_min_periods,
    ).sum()
    weighted_close_location = (
        (close_location * volume).shift(1).rolling(
            spec.close_acceptance_window,
            min_periods=spec.close_acceptance_min_periods,
        ).sum()
        / prior_window_volume
    )
    unweighted_close_location = close_location.shift(1).rolling(
        spec.close_acceptance_window,
        min_periods=spec.close_acceptance_min_periods,
    ).mean()
    prior_volume_weighted_close_location = weighted_close_location
    prior_unweighted_close_location = unweighted_close_location
    prior_volume_close_acceptance = (
        prior_volume_weighted_close_location - prior_unweighted_close_location
    )
    prior_volume_ratio = volume_ratio.shift(1).rolling(
        spec.close_acceptance_window,
        min_periods=spec.close_acceptance_min_periods,
    ).mean()
    acceptance_ready = (
        prior_window_volume.notna()
        & prior_volume_average.shift(1).notna()
        & prior_volume_ratio.notna()
    )
    prior_volume_weighted_close_location = prior_volume_weighted_close_location.where(
        acceptance_ready
    )
    prior_unweighted_close_location = prior_unweighted_close_location.where(acceptance_ready)
    prior_volume_close_acceptance = prior_volume_close_acceptance.where(acceptance_ready)
    prior_volume_ratio = prior_volume_ratio.where(acceptance_ready)
    ...
    volume_close_acceptance_lead = (
        (prior_volume_weighted_close_location >= spec.volume_weighted_close_location_min)
        & (prior_volume_close_acceptance >= spec.close_acceptance_spread_min)
    ...
    if spec.volume_lead_enabled and spec.close_acceptance_enabled:
        raw_signal &= volume_close_acceptance_lead
    result = clean.copy()
    result["sma_20"] = sma
    result["rsi_2"] = rsi
    ...
    result["weighted_close_location"] = weighted_close_location
    result["unweighted_close_location"] = unweighted_close_location
    result["prior_volume_weighted_close_location"] = prior_volume_weighted_close_location
    result["prior_unweighted_close_location"] = prior_unweighted_close_location
    result["prior_volume_close_acceptance"] = prior_volume_close_acceptance
    ...
    result["volume_close_acceptance_lead"] = volume_close_acceptance_lead
