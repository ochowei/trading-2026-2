"""TSM 布林位置加收盤止跌的均值回歸候選。

布林通道採 20 日均線與母體標準差（ddof=0），上下軌為均線加減兩個標準差。
收盤位置 %b <= 0.25（收盤 <= 均線減一個標準差）代表相對跌深；保留既有
成交量先行 1.05 與 Close > prior Close。RSI 與固定跌幅只計算為診斷，
不參與候選或簡單 baseline 篩選。成交、風險、10 日持有與 5 日冷卻沿用 v009。
"""
...
def signal_definition(spec: StrategySpec = DEFAULT_SPEC) -> dict[str, object]:
    ...
    return {
        ...
        "mean_reversion": {
            "close_vs_sma20_gap_minimum": decimal_text(spec.mean_reversion_min),
            "rsi_length": spec.rsi_lookback,
            "rsi_maximum": decimal_text(spec.rsi_max),
            "sma_length": spec.sma_lookback,
            "legacy_oversold_filter_enabled": spec.legacy_oversold_filter_enabled,
            "purpose": "SMA 是布林中線；固定跌幅與 RSI 僅作診斷，不作進場篩選。",
        },
        "bollinger": {
            "lookback": spec.sma_lookback,
            "min_periods": spec.sma_min_periods,
            "standard_deviation_ddof": spec.bollinger_ddof,
            "standard_deviation_multiplier": decimal_text(spec.bollinger_std_multiplier),
            "percent_b_operator": "<=",
            "percent_b_maximum": decimal_text(spec.bollinger_percent_b_max),
            "zero_bandwidth_signal": False,
        },
        ...
        "volume_leads_price": {
            ...
            "volume_ratio_minimum": decimal_text(spec.volume_spike_ratio),
        },
    }
...
def indicators(bars: pd.DataFrame, spec: StrategySpec = DEFAULT_SPEC) -> pd.DataFrame:
    ...
    sma = clean["Close"].rolling(spec.sma_lookback, min_periods=spec.sma_min_periods).mean()
    if spec.bollinger_std_multiplier <= 0 or spec.bollinger_ddof != 0:
        raise ValueError("布林通道採正倍數與母體標準差 ddof=0")
    if spec.legacy_oversold_filter_enabled:
        raise ValueError("本候選不得啟用舊 SMA 跌幅與 RSI 篩選")
    std = (
        clean["Close"]
        .rolling(spec.sma_lookback, min_periods=spec.sma_min_periods)
        .std(ddof=spec.bollinger_ddof)
    )
    lower = sma - spec.bollinger_std_multiplier * std
    upper = sma + spec.bollinger_std_multiplier * std
    percent_b = ((clean["Close"] - lower) / (upper - lower)).where(std > 0, np.nan)
    ...
    raw_signal = percent_b.notna() & (percent_b <= spec.bollinger_percent_b_max)
...
def _intraday_exit(bar: pd.Series, target: float, stop: float) -> tuple[float, ExitReason] | None:
    ...
    *,
    ...
    for session, bar in clean.iterrows():
        if next_trade is not None and next_trade.entry_session == session:
            ...
            entry_fee = (
                next_trade.shares * next_trade.executed_entry_price * cost.fee_bps / 10_000.0
            )
            position = {
                "trade": next_trade,
