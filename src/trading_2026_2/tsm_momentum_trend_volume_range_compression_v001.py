"""TSM v001「量能先行的窄幅壓縮」研究引擎。

本模組測試一個尚未在 TSM 動能趨勢分支使用的量價順序：訊號日前五個已完成
session 的成交量，是否先集中在較窄的日內高低價區間，形成高參與度但低價格
展幅的壓縮狀態，再由訊號日的價格加速確認動能釋放。指標使用原始成交量加權
的日內區間相對於同窗未加權平均區間；它不等同於高量小收盤變動、量能加權收盤
位置、量能加權報酬或高量大區間效率。
...
class StrategySpec:
    ...
    compression_min_periods: int = 5
    compression_ratio_max: float = 1.05
    volume_range_compression_enabled: bool = True
    # Compatibility names preserve the common prior-session boundary contract;
    # the tested mechanism is the range-compression ratio above.
...
BASELINE_SPEC = DEFAULT_SPEC.with_changes(
    volume_range_compression_enabled=False,
    volume_lead_enabled=False,
)
RANGE_COMPRESSION_ONLY_SPEC = DEFAULT_SPEC.with_changes(
...
def _required_history_sessions(spec: StrategySpec) -> int:
    ...
    return max(
        ...
        spec.volume_average_min_periods + spec.compression_window,
...
def indicators(bars: pd.DataFrame, spec: StrategySpec = DEFAULT_SPEC) -> pd.DataFrame:
    ...
    prior_volume_sum = volume.shift(1).rolling(
        spec.compression_window,
        min_periods=spec.compression_min_periods,
    ).sum()
    prior_volume_weighted_range_fraction = (
        (intraday_range_fraction * volume).shift(1).rolling(
            spec.compression_window,
            min_periods=spec.compression_min_periods,
        ).sum()
        / prior_volume_sum
    )
    prior_unweighted_range_fraction = intraday_range_fraction.shift(1).rolling(
        spec.compression_window,
        min_periods=spec.compression_min_periods,
    ).mean()
    prior_volume_range_compression = (
        prior_volume_weighted_range_fraction / prior_unweighted_range_fraction
    ).where(prior_unweighted_range_fraction > 0)
    ...
    compression_ready = (
        ...
        & prior_volume_range_compression.notna()
        & prior_volume_ratio.notna()
    )
    prior_volume_weighted_range_fraction = prior_volume_weighted_range_fraction.where(
        compression_ready
    )
    prior_unweighted_range_fraction = prior_unweighted_range_fraction.where(compression_ready)
    prior_volume_range_compression = prior_volume_range_compression.where(compression_ready)
    prior_volume_ratio = prior_volume_ratio.where(compression_ready)
    ...
    compression_qualifies = (
        (prior_volume_range_compression <= spec.compression_ratio_max)
        & (prior_volume_ratio >= spec.volume_spike_ratio)
    )
    volume_range_compression_lead = (
        compression_qualifies
        if spec.volume_range_compression_enabled and spec.volume_lead_enabled
    ...
    if spec.volume_range_compression_enabled and spec.volume_lead_enabled:
        raw_signal &= volume_range_compression_lead
    result = clean.copy()
    result["sma_20"] = sma
    result["rsi_2"] = rsi
    ...
    result["prior_unweighted_range_fraction"] = prior_unweighted_range_fraction
    result["prior_volume_range_compression"] = prior_volume_range_compression
    ...
    result["volume_range_compression_lead"] = volume_range_compression_lead
