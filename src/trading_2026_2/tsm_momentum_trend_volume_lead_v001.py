"""TSM v001「分散量能壓力先於價格加速」研究引擎。

本模組測試一個和單日放量後固定價突破不同的機制：在趨勢仍然成立時，
先觀察最近五個已完成 session 的累積成交量是否集中，且上漲或持平日
是否承擔主要成交量；當日只需出現價格加速，就把這段分散的量能壓力
視為價格動能可能延續的先行訊號。
...
BASELINE_SPEC = DEFAULT_SPEC.with_changes(
    volume_pressure_enabled=False,
    volume_lead_enabled=False,
)
VOLUME_PRESSURE_ONLY_SPEC = DEFAULT_SPEC.with_changes(
    require_close_above_prior_close=False,
    volume_pressure_enabled=True,
)
...
def _required_history_sessions(spec: StrategySpec) -> int:
    ...
    return max(
        ...
        spec.volume_average_min_periods + spec.pressure_lookback,
...
def indicators(bars: pd.DataFrame, spec: StrategySpec = DEFAULT_SPEC) -> pd.DataFrame:
    ...
    prior_volume_sum = volume.shift(1).rolling(
        spec.pressure_lookback,
        min_periods=spec.pressure_lookback,
    ).sum()
    prior_close = close.shift(1)
    up_or_flat_day = close.ge(prior_close)
    up_volume = volume.where(up_or_flat_day, 0.0)
    prior_up_volume_sum = up_volume.shift(1).rolling(
        spec.pressure_lookback,
        min_periods=spec.pressure_lookback,
    ).sum()
    prior_volume_concentration = prior_volume_sum / (
        prior_volume_average * spec.pressure_lookback
    )
    prior_up_volume_share = prior_up_volume_sum / prior_volume_sum
    ...
    pressure_lead = (
        (prior_volume_concentration >= spec.pressure_concentration_min)
        & (prior_up_volume_share >= spec.pressure_up_volume_share_min)
    )
    momentum_signal = (
        trend_regime
    ...
    if spec.volume_pressure_enabled:
        raw_signal &= pressure_lead
    result = clean.copy()
    result["sma_20"] = sma
    result["rsi_2"] = rsi
    result["prior_volume_average"] = prior_volume_average
    result["prior_volume_sum"] = prior_volume_sum
    result["prior_up_volume_sum"] = prior_up_volume_sum
    result["prior_volume_concentration"] = prior_volume_concentration
    result["prior_up_volume_share"] = prior_up_volume_share
    ...
    result["pressure_lead"] = pressure_lead
    result["raw_momentum_signal"] = momentum_signal
    result["raw_signal"] = raw_signal.fillna(False).astype(bool)
    result["prior_volume_spike_ratio"] = volume_spike_ratio.shift(1).rolling(
