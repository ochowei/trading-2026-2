"""TSM v004「量能加權隔夜跳空壓力」研究引擎。

本模組測試一個尚未在 TSM 動能趨勢分支使用的量價順序：訊號日前五個已完成
session 的成交量，是否集中在開盤相對前一日收盤走弱的隔夜缺口，形成量能先行
的跳空壓力，再由訊號日價格加速確認反彈動能延續。缺口指標只讀取訊號日前的
Open 與前一日 Close，不使用訊號日的資料。
...
class StrategySpec:
    ...
    volume_weighted_gap_max: float = -0.002
    negative_gap_volume_share_min: float = 0.10
    volume_spike_ratio: float = 1.05
    volume_gap_anchoring_enabled: bool = True
...
def indicators(bars: pd.DataFrame, spec: StrategySpec = DEFAULT_SPEC) -> pd.DataFrame:
    """計算只使用當日以前資訊的趨勢、價格加速及隔夜缺口量能壓力指標。"""
    ...
    overnight_gap = open_price / prior_close - 1.0
    ...
    prior_volume_weighted_gap = (
        (overnight_gap * volume).shift(1).rolling(
            spec.volume_lead_window,
            min_periods=spec.volume_lead_min_periods,
        ).sum()
        / prior_volume_sum
    )
    negative_gap_volume = (volume * (overnight_gap < 0).astype(float)).shift(1).rolling(
        spec.volume_lead_window,
        min_periods=spec.volume_lead_min_periods,
    ).sum()
    prior_negative_gap_volume_share = negative_gap_volume / prior_volume_sum
    ...
    gap_ready = (
        ...
        & prior_volume_weighted_gap.notna()
        & prior_negative_gap_volume_share.notna()
        & prior_volume_ratio.notna()
    )
    prior_volume_weighted_gap = prior_volume_weighted_gap.where(gap_ready)
    prior_negative_gap_volume_share = prior_negative_gap_volume_share.where(gap_ready)
    prior_volume_ratio = prior_volume_ratio.where(gap_ready)
    ...
    gap_qualifies = (
        (prior_volume_weighted_gap <= spec.volume_weighted_gap_max)
        & (prior_negative_gap_volume_share >= spec.negative_gap_volume_share_min)
        & (prior_volume_ratio >= spec.volume_spike_ratio)
    )
    volume_gap_anchoring_lead = (
        gap_qualifies
        if spec.volume_gap_anchoring_enabled and spec.volume_lead_enabled
    ...
    if spec.volume_gap_anchoring_enabled and spec.volume_lead_enabled:
        raw_signal &= volume_gap_anchoring_lead
    result = clean.copy()
    result["sma_20"] = sma
    result["rsi_2"] = rsi
    result["prior_volume_average"] = prior_volume_average
    result["volume_spike_ratio"] = volume_spike_ratio
    result["overnight_gap"] = overnight_gap
    result["prior_volume_weighted_gap"] = prior_volume_weighted_gap
    result["prior_negative_gap_volume_share"] = prior_negative_gap_volume_share
    ...
    result["volume_gap_anchoring_lead"] = volume_gap_anchoring_lead
    ...
    *,
    ...
    for index, (session, bar) in enumerate(frame.iterrows()):
        if position is not None and time_exit_index == index:
            state = position
            close_position(session, float(bar["Open"]), "time", spec.holding_sessions, state)
