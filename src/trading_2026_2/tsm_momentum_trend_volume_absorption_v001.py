"""TSM v001「量能吸收先於價格釋放」研究引擎。

本模組測試一個和既有「多日量能集中」或「單次量能脈衝」不同的量價順序：
訊號日前五個已完成 session 內，先出現高成交量但收盤變化很小的「吸收」
狀態，之後才由訊號日的價格加速確認動能釋放。候選觀察的是量能與價格
結果的背離，不是只計算量能總量或單純尋找量能尖峰。
...
class StrategySpec:
    ...
    absorption_min_periods: int = 5
    absorption_volume_ratio: float = 1.20
    absorption_price_move_max: float = 0.005
    volume_absorption_enabled: bool = True
    # v003 contract compatibility names; the implementation semantics are the
    # absorption fields above, not a pulse or a raw volume spike.
...
BASELINE_SPEC = DEFAULT_SPEC.with_changes(
    volume_absorption_enabled=False,
    volume_lead_enabled=False,
)
VOLUME_ABSORPTION_ONLY_SPEC = DEFAULT_SPEC.with_changes(
...
def _required_history_sessions(spec: StrategySpec) -> int:
    ...
    return max(
        ...
        spec.volume_average_min_periods + spec.absorption_window,
...
def indicators(bars: pd.DataFrame, spec: StrategySpec = DEFAULT_SPEC) -> pd.DataFrame:
    ...
    absorption_session = (
        (volume_spike_ratio >= spec.absorption_volume_ratio)
        & (price_move <= spec.absorption_price_move_max)
    )
    prior_absorption_count = absorption_session.shift(1).rolling(
        spec.absorption_window,
        min_periods=spec.absorption_min_periods,
    ).sum()
    prior_absorption_ready = volume_spike_ratio.shift(1).rolling(
        spec.absorption_window,
        min_periods=spec.absorption_min_periods,
    ).count() >= spec.absorption_min_periods
    prior_absorption_count = prior_absorption_count.where(prior_absorption_ready)
    ...
    volume_absorption_lead = prior_absorption_count >= 1
    momentum_signal = (
        trend_regime
    ...
    if spec.volume_absorption_enabled:
        raw_signal &= volume_absorption_lead
    result = clean.copy()
    result["sma_20"] = sma
    result["rsi_2"] = rsi
    ...
    result["absorption_session"] = absorption_session
    result["prior_absorption_count"] = prior_absorption_count
    result["prior_volume_spike_ratio"] = prior_absorption_count
    ...
    result["volume_absorption_lead"] = volume_absorption_lead
    result["volume_pulse_lead"] = volume_absorption_lead
