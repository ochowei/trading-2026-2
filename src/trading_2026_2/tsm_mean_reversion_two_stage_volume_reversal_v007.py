"""TSM「價行止跌」時間衰減版短線均值回歸研究的可重算核心。

本模組只接受已凍結的日線 OHLCV，不下載資料、不連線券商，也不建立真實委託。
候選整合 20 日均線負偏離（>= 1.5%）、訊號日反轉收紅（Close > prior Close）
與反彈後仍低於 50 的 2 日 RSI；相較 v006，本輪唯一的策略變更是移除「訊號日前
5 個 session 內任一成交量尖峰」這個條件，直接檢查價行止跌是否足以形成可重現的
均值回歸 setup。反轉優勢若在 10 個完整持有 session 內沒有實現，就於下一個 open
以 time exit 退出。baseline 再關閉價格反轉條件，保留相同執行規則。
"""
...
FULL_TWO_STAGE_SPEC = DEFAULT_SPEC.with_changes(volume_lead_enabled=True)
VOLUME_ONLY_SPEC = FULL_TWO_STAGE_SPEC.with_changes(require_close_above_prior_close=False)
