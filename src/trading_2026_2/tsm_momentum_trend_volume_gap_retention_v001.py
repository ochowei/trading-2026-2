"""TSM v004「前五日量比先行、訊號日正向缺口收盤接受」研究引擎。

candidate 要求訊號日前五個完整 session 的平均量比至少 1.05，接著訊號日
開盤須高於前收至少 0.5%，且收盤不低於當日開盤，表示當日收盤仍接受
正向缺口。baseline 保留量比背景，只關閉訊號日缺口接受條件。成交量只代表
整個 session 的總量，不能推論隔夜時段實際成交量。
...
class StrategySpec:
    ...
    signal_gap_min_return: float = 0.005
    volume_spike_ratio: float = 1.05
    signal_day_gap_acceptance_enabled: bool = True
...
BASELINE_SPEC = DEFAULT_SPEC.with_changes(signal_day_gap_acceptance_enabled=False)
...
def indicators(bars: pd.DataFrame, spec: StrategySpec = DEFAULT_SPEC) -> pd.DataFrame:
    ...
    signal_day_gap_accepted = (
        (overnight_gap >= spec.signal_gap_min_return)
        & (clean["Close"] >= open_price)
    )
    ...
    signal_gap_qualifies = (
        signal_day_gap_accepted
        if spec.signal_day_gap_acceptance_enabled
    ...
    if spec.volume_lead_enabled:
        raw_signal &= prior_volume_ratio >= spec.volume_spike_ratio
    raw_signal &= signal_gap_qualifies
    result = clean.copy()
    result["sma_20"] = sma
    result["rsi_2"] = rsi
    ...
    result["signal_day_gap_accepted"] = signal_day_gap_accepted
    ...
    result["signal_gap_qualifies"] = signal_gap_qualifies
