"""TSM「量先於價＋方向確認」短線均值回歸研究的可重算核心。

本模組只接受已凍結的日線 OHLCV，不下載資料、不連線券商，也不建立真實委託。
候選延續 20 日均線偏離、RSI(2) 超賣與成交量先行條件，並明確要求訊號日
收盤低於前一個 XNYS session 的收盤，避免把沒有方向的成交量 setup 直接當成
可交易 trigger。SMA＋RSI baseline 透過關閉兩個額外條件，沿用完全相同的執行規則。
"""
...
class StrategySpec:
    ...
    require_close_below_prior_close: bool = True
...
BASELINE_SPEC = DEFAULT_SPEC.with_changes(
    ...
    require_close_below_prior_close=False,
)
...
def indicators(bars: pd.DataFrame, spec: StrategySpec = DEFAULT_SPEC) -> pd.DataFrame:
    ...
    close_below_prior_close = clean["Close"] < prior_close
    ...
    result["close_below_prior_close"] = close_below_prior_close
    raw_signal = (
    ...
    if spec.require_close_below_prior_close:
        raw_signal &= result["close_below_prior_close"]
...
def backtest(
    ...
    *,
    ...
    ``reset_at_start`` 用於每個 Evaluation fold 和 2025 replay：前 25 個
