"""TSM v001「短窗量能脈衝先於價格加速」研究引擎。

本模組測試一個和既有「多日量能集中」或「單日放量事件後突破」不同的
量價順序：在趨勢仍然成立時，先觀察訊號日前三個已完成 session 內是否
出現高於近二十日均量的量能脈衝；當日才用價格加速確認動能延續。成交量
脈衝的時間順序是候選機制本身，不以五日總量或事件後固定價突破取代。
...
class StrategySpec:
    ...
    volume_lead_window: int = 3
    volume_lead_min_periods: int = 3
...
VOLUME_RAMP_ONLY_SPEC = DEFAULT_SPEC.with_changes(
...
def indicators(bars: pd.DataFrame, spec: StrategySpec = DEFAULT_SPEC) -> pd.DataFrame:
    ...
    prior_volume_spike_ratio = volume_spike_ratio.shift(1).rolling(
    ...
    volume_pulse_lead = prior_volume_spike_ratio >= spec.volume_spike_ratio
    momentum_signal = (
        trend_regime
    ...
    result["volume_pulse_lead"] = volume_pulse_lead
