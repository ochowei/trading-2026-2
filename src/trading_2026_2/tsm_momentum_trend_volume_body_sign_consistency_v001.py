"""TSM v004「量能集中於非負日內實體」研究引擎。

本模組測試一個與總量壓力、日內實體報酬大小及收盤到收盤報酬不同的
量先價行機制：訊號日前五個已完成 session 中，成交量有多少比例落在
Open 到 Close 不下跌的日內實體。它只看成交量的方向分布，不把實體大小
加權；當日再由既定趨勢、RSI、收盤方向與價格加速確認動能。平盤歸入
非負類別，是事前固定的資料定義，不代表盤中主動買方成交。

所有先行指標只使用訊號日前已完成的日線 OHLCV；模組只處理離線資料，
不下載資料、不連線券商，也不建立真實委託。
"""
...
class StrategySpec(_execution.StrategySpec):
    ...
    volume_body_sign_consistency_enabled: bool = True
    volume_positive_body_share_min: float = 0.60
...
DEFAULT_SPEC = StrategySpec(
    ...
    volume_body_sign_consistency_enabled=True,
    volume_positive_body_share_min=0.60,
)
BASELINE_SPEC = DEFAULT_SPEC.with_changes(
    volume_body_sign_consistency_enabled=False,
)
...
def indicators(
    ...
    # 平盤與上漲日都屬於非負日；訊號日仍只讀取前五個完成 session。
    positive_body_volume = volume.where(body_return >= 0.0, 0.0)
    prior_positive_body_volume = positive_body_volume.shift(1).rolling(
        spec.volume_lead_window,
        min_periods=spec.volume_lead_min_periods,
    ).sum()
    prior_positive_body_volume_share = (
        prior_positive_body_volume / prior_volume_sum
    )
    body_ready = (
        prior_volume_ratio.notna()
        & prior_positive_body_volume_share.notna()
    )
    prior_volume_ratio = prior_volume_ratio.where(body_ready)
    prior_positive_body_volume_share = prior_positive_body_volume_share.where(
        body_ready
    )
    ...
    volume_body_sign_consistency_lead = (
        prior_positive_body_volume_share >= spec.volume_positive_body_share_min
        if spec.volume_body_sign_consistency_enabled
        else pd.Series(True, index=clean.index)
    )
    volume_lead = (
        volume_ratio_lead & volume_body_sign_consistency_lead
    ...
    result["prior_positive_body_volume"] = prior_positive_body_volume
    result["prior_positive_body_volume_share"] = prior_positive_body_volume_share
    ...
    result["volume_body_sign_consistency_lead"] = (
        volume_body_sign_consistency_lead
    )
