"""TSM「兩階段量先價行」適度換手版短線均值回歸研究的可重算核心。

本模組只接受已凍結的日線 OHLCV，不下載資料、不連線券商，也不建立真實委託。
候選整合 20 日均線負偏離（>= 1.5%）、訊號日反轉收紅（Close > prior Close）、
反彈後仍低於 50 的 2 日 RSI，以及前 5 個 session 內出現成交量溫和放大
（Volume / SMA20(Volume.shift(1)) >= 1.05）的量先特徵。
相較 v006 採用 1.25 倍尖峰門檻導致交易樣本過度緊縮（僅 20 筆），以及 v007
完全關閉成交量濾網導致假止跌跳空停損侵蝕 Profit Factor，本輪（v008）重新
啟用成交量領先確認，並將尖峰門檻適度校準為 1.05 倍，在保留足夠交易樣本的
同時濾除無量弱勢假反彈。10 個完整持有 session 內未實現停損或停利的部位於下一個
open 以 time exit 退出。baseline 關閉成交量先行與價格反轉條件，保留相同執行規則。
"""
