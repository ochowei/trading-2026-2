...
from trading_2026_2 import tsm_momentum_trend_volume_absorption_v001 as strategy
...
def make_bars(rows: int = 180, signal_index: int = 40) -> pd.DataFrame:
    ...
    volume[signal_index - 2] = 1_250_000.0
    volume[signal_index - 1] = 1_000_000.0
    return pd.DataFrame(
        {
            "Open": open_price,
            "High": high,
...
def test_volume_absorption_precedes_price_acceleration() -> None:
    bars = make_bars()
    ...
    assert signal["prior_absorption_count"] >= 1
    assert bool(signal["volume_absorption_lead"])
...
def test_volume_ramp_columns_do_not_use_current_volume() -> None:
    ...
    for column in ("prior_absorption_count", "volume_absorption_lead", "raw_signal"):
        assert before[column] == after[column]


def test_baseline_is_same_price_path_without_volume_absorption() -> None:
    bars = make_bars()
    ...
    assert not bool(candidate.iloc[40]["volume_absorption_lead"])
