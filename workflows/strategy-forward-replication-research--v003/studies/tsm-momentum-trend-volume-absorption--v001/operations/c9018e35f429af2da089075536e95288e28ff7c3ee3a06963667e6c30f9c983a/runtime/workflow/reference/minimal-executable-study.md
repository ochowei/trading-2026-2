...
v003 的實際建立與凍結前操作請先依 [操作指南](operations.md) 執行；以下說明研究階段與保留的語意。

本文件將 `strategy-forward-replication-research` 的研究流程縮減為最小可執行規格。詳細規則與
解釋請參考 [`strategy-forward-replication-research-v003-guide.md`](strategy-forward-replication-research-v003-guide.md)。
...
- `indeterminate`：identity、資料、artifact、派工、provenance 或 integrity 不足以可信判定。
