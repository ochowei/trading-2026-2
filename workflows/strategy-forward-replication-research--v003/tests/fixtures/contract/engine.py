"""TSM「量先於價＋反轉觸發」短線均值回歸研究的可重算核心。

本模組只接受已凍結的日線 OHLCV，不下載資料、不連線券商，也不建立真實委託。
候選延續 20 日均線偏離、RSI(2) 超賣與成交量先行條件，並明確要求訊號日
收盤高於前一個 XNYS session 的收盤，將止跌反轉作為可交易 trigger。SMA＋RSI baseline
透過關閉兩個額外條件，沿用完全相同的執行規則。
"""
...
def indicators(bars: pd.DataFrame, spec: StrategySpec = DEFAULT_SPEC) -> pd.DataFrame:
    ...
    raw_signal = (result["mean_reversion_gap"] >= spec.mean_reversion_min) & (
