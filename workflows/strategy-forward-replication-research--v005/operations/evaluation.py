...
from operations.multi_asset import request_assets, snapshot_identity, validate_assets
...
def historical_evaluation(service, study_id, plan):
    ...
    if multi and plan["data_digest"] != canonical_digest(
        [snapshot_identity(asset) for asset in plan["data_assets"]]
    ):
        raise IntegrityError("Evaluation 多資產總 digest 與逐檔資料不一致")
    with StudyLock(service.study_root(study_id) / ".writer.lock"):
        ...
        if state.effective_event_type == "historical-evaluation-started":
            ...
            evaluation_snapshot = next(
                item for item in snapshot["snapshots"] if item["role"] == "historical-evaluation"
            )
            if multi:
                assets = plan["data_assets"]
                if [snapshot_identity(asset) for asset in assets] != evaluation_snapshot.get("assets"):
                    raise IntegrityError("Evaluation 資產清單與 frozen snapshot 不一致")
                checked = validate_assets(
                    service.repository_root, assets,
                    service.rules.workflow["data_intervals"]["intervals"],
                    stage="historical-evaluation",
                )
                if assets[0]["start_date"] != evaluation_snapshot["sessions"][0] or assets[0]["end_date"] != evaluation_snapshot["sessions"][-1]:
                    raise IntegrityError("Evaluation rows 與 frozen session inventory 不一致")
                access = {"assets": [
                    {"asset_id": asset["asset_id"], "path": asset["data_path"],
                     "expected_digest": asset["data_digest"]} for asset in assets
                ]}
            else:
                safe_path(
                ...
                if dates != evaluation_snapshot["sessions"]:
                    raise IntegrityError("Evaluation rows 與 frozen session inventory 不一致")
            ...
            if multi:
                request["data_assets"] = request_assets(assets)
                request["availability_policy"] = "after-close-next-session"
            else:
                request["data_path"] = "run/bars.csv"
                request["data_digest"] = plan["data_digest"]
