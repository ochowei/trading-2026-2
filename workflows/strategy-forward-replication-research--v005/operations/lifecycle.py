...
from operations.multi_asset import request_assets, validate_assets
...
def development(service, study_id, plan):
    ...
    with StudyLock(root / ".writer.lock"):
        ...
        output = runtime / "evidence.yml"
        runtime_manifest_path = runtime / "runtime-manifest.yml"
        if output.exists():
            manifest = load_runtime_manifest(runtime_manifest_path, service)
            if manifest["evidence"].get("digest") != canonical_digest(output.read_bytes()):
                raise IntegrityError("Development runtime evidence digest 不一致")
        if not output.exists():
            ...
            if multi:
                assets = plan["data_assets"]
                if inputs.get("data_bindings", {}).get("assets") != assets:
                    raise IntegrityError("多資產清單與 frozen trial inputs 不一致")
                checked = validate_assets(
                    service.repository_root,
                    assets,
                    service.rules.workflow["data_intervals"]["intervals"],
                )
                if any(asset["start_date"] < start or asset["end_date"] > end for asset in assets):
                    raise ValidationError("多資產日期超出 frozen Development 範圍")
                access = {
                    "assets": [
                        {"asset_id": asset["asset_id"], "path": asset["data_path"],
                         "expected_digest": asset["data_digest"]}
                        for asset in assets
                    ]
                }
                request = {
                    "stage": "development",
                    "data_assets": request_assets(assets),
                    "availability_policy": "after-close-next-session",
                ...
                access = {"path": plan["data_path"], "expected_digest": plan["data_digest"]}
                request = {
                    "stage": "development",
            ...
            atomic_create(directory / "data-access.yml", canonical_bytes(access))
            service.rules.schema_store.validate("runner-request.schema.yml", request)
            runner = load_canonical(research / "runner-contract.yml")["runners"]["development"]
            ...
            publish_runtime_manifest(
                ...
                make_runtime_manifest(
                    ...
                    request,
                    plan.get("data_path", ""),
                    plan.get("data_digest", ""),
                    "runtime/evidence.yml",
                    data_assets=assets if multi else None,
                ),
            )
            atomic_create(
            ...
            with tempfile.TemporaryDirectory(prefix="development-execution-") as temp:
                ...
                if multi:
                    ...
                    asset_dir.mkdir()
                    for asset, asset_data in checked:
                        (asset_dir / f"{asset['asset_id']}.csv").write_bytes(asset_data)
                else:
                    (run / "bars.csv").write_bytes(data)
                request_path = run / "request.yml"
                request_path.write_bytes(canonical_bytes(request))
                temp_output = run / "evidence.yml"
                launch(isolated, staged_package, runner, request_path, temp_output)
                value = load_canonical(temp_output)
                atomic_create(output, canonical_bytes(value))
                manifest = make_runtime_manifest(
                    ...
                    plan.get("data_digest", ""),
                    "runtime/evidence.yml",
                    canonical_digest(temp_output.read_bytes()),
                    data_assets=assets if multi else None,
                )
...
def freeze_steps(service, study_id, plan):
    ...
    selected_inputs = load_canonical(
        service.study_root(study_id) / state.trials[selected]["inputs_path"]
    )
    development_assets = selected_inputs.get("data_bindings", {}).get("assets")
    evaluation_snapshot = snapshots["historical-evaluation"]
    if development_assets is not None:
        ...
        actual_roster = [
            (asset["asset_id"], asset["use"], asset["provider"], asset["symbol"])
            for asset in evaluation_snapshot.get("assets", [])
        ]
        if actual_roster != expected_roster:
            raise IntegrityError("凍結的資產清單與 Development Trial 不一致")
    elif "assets" in evaluation_snapshot:
    ...
    # Snapshot metadata 與 session inventory 已檢查；不讀 Evaluation 價格或結果。
...
def freeze_readiness(service, study_id, plan):
    ...
    with tempfile.TemporaryDirectory(prefix="freeze-readiness-") as temp:
        staged_repository = Path(temp) / "repository"
        package = staged_repository / "workflows" / service.workflow_root.name
        ...
        authority = staged_repository / ".authority"
