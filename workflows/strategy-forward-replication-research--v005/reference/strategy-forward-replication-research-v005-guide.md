...
本文件說明 `strategy-forward-replication-research--v005` 的操作方式。行為權威是同一 Workflow Package 內，由 `release-manifest.yml` 綁定的 `workflow.yml`、`rules/`、`schemas/`、`policies/`、`validator/` 與 `writer/`。本版尚未發布，Release Candidate 不代表 Active。正式研究不得使用 `--allow-draft` 規避 release 驗證。
...
```text
workflows/strategy-forward-replication-research--v005/studies/<study-id>/
```
...
Study 建立時另外保存 `manifests/workflow-reference.yml`。它只記錄 v005 Workflow Package 的 repository-relative 路徑與 digest；Study 不在 operation runtime 內複製 `runtime/workflow/`。執行所需的 Workflow、Source Bundle 與 runner 會進入隔離暫存 workspace，永久 runtime 只保存 request、runtime manifest 及 evidence／artifact 引用。
...
多資產 Study 在四種資料角色內都列出同一組資產 ID、用途、來源與標的，各角色逐資產有自己的 digest；具體輸入與可得時間規則見[多資產資料契約](multi-asset-input.md)。
...
python workflows/strategy-forward-replication-research--v005/operations/cli.py \
  --repository-root <repo> --authority-root <authority> --role 'study 開發者' \
  prepare <study-id> --report <report>
python workflows/strategy-forward-replication-research--v005/operations/cli.py \
  --repository-root <repo> --authority-root <authority> --role 'study 開發者' \
  development <study-id> --plan <plan> --assignment <assignment>
```
