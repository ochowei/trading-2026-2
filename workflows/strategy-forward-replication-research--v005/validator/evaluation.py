...
from operations.multi_asset import snapshot_identity
...
def verify_completed(root, state, payload, rules):
    ...
    runtime = resolve_inside(directory, "runtime")
    runtime_manifest = load_canonical(resolve_inside(runtime, "runtime-manifest.yml"))
    request = load_canonical(resolve_inside(runtime, "request.yml"))
    request_digest = (
        canonical_digest([snapshot_identity(asset) for asset in request["data_assets"]])
        if "data_assets" in request else request["data_digest"]
    )
    if (
        marker["request_digest"] != canonical_digest(request)
        or request_digest != operation["data_digest"]
        or request["preregistration"] != state.preregistration
    ):
        raise IntegrityError("runner request 與啟動內容不一致")
    if (
        payload["evidence_path"] != runtime_manifest["evidence"]["path"]
        or payload["evidence_digest"] != runtime_manifest["evidence"].get("digest")
    ):
        raise IntegrityError("runtime manifest 與 Evaluation evidence 不一致")
    ...
    if payload["evidence_digest"] != canonical_digest(evidence):
