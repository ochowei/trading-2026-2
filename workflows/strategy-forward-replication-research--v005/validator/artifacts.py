    ...
    *,
    ...
    try:
        in_historical_store = bool(
            ...
            and is_within_repository_path(relative_path, historical_evaluation_artifacts_path)
        )
    ...
    for trade in trades:
        ...
        for model, pnl in (("base", base_pnl), ("stress", stress_pnl)):
            ...
            if pnl < 0 and equity > 0:
                maximum_realized_loss_fraction = max(maximum_realized_loss_fraction, -pnl / equity)
...
def _development_bootstrap(
    ...
    *,
    ...
    return {
        ...
        "seed": seed,
        "return_q05_q50_q95": [
            _development_text(value) for value in np.quantile(returns, [0.05, 0.5, 0.95])
        ],
        "profit_factor_q05_q50_q95": [
            _development_text(value) for value in np.quantile(profit_factors, [0.05, 0.5, 0.95])
        ],
        "maximum_drawdown_q05_q50_q95": [
            _development_text(value) for value in np.quantile(maximum_drawdown, [0.05, 0.5, 0.95])
        ],
        "positive_return_ratio": _development_text(np.mean(returns > 0)),
        "profit_factor_above_one_ratio": _development_text(np.mean(profit_factors > 1)),
        "drawdown_above_10pct_ratio": _development_text(np.mean(maximum_drawdown > 0.10)),
    }
...
def _development_model_metrics(
    ...
    for trade in trades:
        ...
        rate = pnl / equity
        if not _development_numeric_equal(detail["pnl_fraction_of_pre_entry_equity"], rate):
            raise IntegrityError(
    ...
    metrics = {
        ...
        "profit_factor": _development_text(gross_profit / gross_loss if gross_loss else inf),
...
def _recompute_development(
    ...
    base_metrics, base_rates, base_pnls = _development_model_metrics(trades, "base", initial_cash)
    ...
    for year, base_pnl, stress_pnl in zip(signal_years, base_pnls, stress_pnls, strict=True):
...
def empty_development_result(preregistration: dict[str, Any]) -> dict[str, Any]:
    """無交易無法估計抽樣統計；明示不可估計並拒絕候選資格。"""
    rules = preregistration["eligibility_rules"]["development_gates"]
    gates = []
    for name, rule in rules.items():
        actual = 0 if name in {"completed_trades", "traded_years"} else None
        gates.append(
            {
                "gate": name,
                ...
                "passed": False
                if actual is None
                else compare(actual, rule["operator"], rule["value"], metric=name),
            }
        )
    failed = [item["gate"] for item in gates if not item["passed"]]
    return {
        "metrics": {"completed_trades": 0, "traded_years": 0},
        "diagnostics": {"status": "not-estimable", "reason": "no-trades"},
        "accepted_signal_count": 0,
        "gates": gates,
        "failed_gates": failed,
        "disposition": "fail",
        "qualification_failure": "no-trades",
    }
...
def validate_development_trial(
    ...
    *,
    ...
    if assets is not None:
        expected_bindings["data_assets_digest"] = canonical_digest(assets)
    ...
    if value["trades"] == []:
        expected = empty_development_result(preregistration)
        for key, wanted in expected.items():
            if value.get(key) != wanted:
                raise IntegrityError(f"無交易 Development {key} 與重算結果不一致")
        return [*expected["failed_gates"], "no-trades"]

    metrics, diagnostics, recomputed_actuals = _recompute_development(value, preregistration)
    ...
    for item in gate_records:
        ...
        passed = compare(recomputed_actuals[name], rule["operator"], rule["value"], metric=name)
...
def validate_snapshot_set(
    ...
    optional_roles = set(data_intervals.get("legacy_optional_roles", optional_intervals))
    ...
    for snapshot in snapshots:
        ...
        if "assets" in snapshot:
            ...
            if ids != sorted(set(ids)) or sum(asset["use"] == "trade" for asset in assets) != 1:
                raise ValidationError("Snapshot 資產 ID 須唯一排序，且恰有一個交易標的")
            if sorted(asset["symbol"] for asset in assets) != sorted(snapshot["symbols"]):
                raise IntegrityError("Snapshot symbols 與逐資產標的不一致")
            if any(asset["provider"] != snapshot["provider"] or asset["timezone"] != snapshot["timezone"] for asset in assets):
                raise IntegrityError("Snapshot 逐資產來源或時區不一致")
            roster = [(asset["asset_id"], asset["use"], asset["provider"], asset["symbol"]) for asset in assets]
            if asset_roster is None:
                asset_roster = roster
            elif roster != asset_roster:
                raise IntegrityError("各資料區間的資產清單不一致")
            if snapshot["data_digest"] != canonical_digest(assets):
                raise IntegrityError("Snapshot 多資產總 digest 不一致")
            if any(asset["start_date"] != snapshot["sessions"][0]
                   or asset["end_date"] != snapshot["sessions"][-1] for asset in assets):
                raise IntegrityError("Snapshot 資產日期界線與 session inventory 不一致")
        elif asset_roster is not None:
            raise ValidationError("多資產 Snapshot 不能混用單資產區間")
    ...
    if any("assets" in snapshot for snapshot in snapshots) and any(
        "assets" not in snapshot for snapshot in snapshots
    ):
        raise ValidationError("多資產 Snapshot 必須涵蓋所有資料角色")
    return result
