...
def main():
    ...
    if "data_assets" in request:
        for asset in request["data_assets"]:
            path = Path(asset["data_path"])
            assert canonical_digest(path.read_bytes()) == asset["data_digest"]
        data = Path(next(asset["data_path"] for asset in request["data_assets"] if asset["use"] == "trade"))
    else:
    ...
    if request["stage"] == "development":
        ...
        for model in ("candidate", "baseline"):
            ...
            if "assets" in inputs.get("data_bindings", {}):
                value["bindings"]["data_assets_digest"] = canonical_digest(
                    inputs["data_bindings"]["assets"]
                )
            if trades:
