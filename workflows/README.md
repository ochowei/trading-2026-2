...
## Strategy Forward Replication Research v003（目前 Active）

詳細內容請參考 [`strategy-forward-replication-research--v003/README.md`](strategy-forward-replication-research--v003/README.md)。這是目前接受新 Study 的 Workflow Package；它以自己的 rules、schemas、validator、writer、policies、tests 與 Release Record 綁定完整研究語意。

- [`release.yml`](strategy-forward-replication-research--v003/release.yml)：Trusted Approver 建立的正式啟用紀錄。
- [`release-manifest.yml`](strategy-forward-replication-research--v003/release-manifest.yml)：v003 權威檔案與 SHA-256 數位指紋清單。
- [`release-test-report.yml`](strategy-forward-replication-research--v003/release-test-report.yml)：v003 發布前測試與完整性驗證報告。

既有 v002 Study 不會自動搬遷或重跑；v002 的 Package、原始 Release Record、既有 Study、evidence 與 authority 均保留，狀態為 `Superseded`。v001 同樣維持 `Superseded`。

## Strategy Forward Replication Research v002（Superseded）

詳細內容請參考 [`strategy-forward-replication-research--v002/README.md`](strategy-forward-replication-research--v002/README.md)。這個 Workflow Package 已由 v003 取代，不再接受新的 Study；它仍保留自己的 rules、schemas、validator、writer、policies、tests 與原始 Release Record，供既有 Study 讀取與獨立驗證。

- [`release.yml`](strategy-forward-replication-research--v002/release.yml)：原始 Trusted Approver 建立的正式啟用紀錄，未被改寫。
...
v002 的既有 Study 不會搬遷、重跑或因 v003 啟用而失效。
