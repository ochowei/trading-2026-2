...
使用 canonical YAML（鍵排序、固定縮排與型別），可用 `validator.canonical_yaml.canonical_bytes` 寫出。`schemas/assignment.schema.yml` 定義必填內容：指令來源 `source_id`、原文或可追查摘錄 `instruction`、指派者 `assigner`、受派者 `assignee`、角色、Study ID、v004 與範圍。
...
命令入口固定為 `workflows/strategy-forward-replication-research--v004/operations/cli.py`。全域參數必須在子命令前：
