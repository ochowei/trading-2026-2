...
v004 的 Study 不再把 Workflow package 複製到自己的 operation runtime。Study 建立時會保存一份 `manifests/workflow-reference.yml`，它是不可變的查找契約，不是 Workflow 的第二份副本。

reference 至少固定以下內容：

- `workflow_package_path`：repository 內 `workflows/strategy-forward-replication-research--v004` 的相對路徑。
