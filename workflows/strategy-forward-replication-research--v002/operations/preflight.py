...
from validator.artifacts import evaluate_historical, validate_development_trial
...
def source_files(repository: Path, bundle: dict) -> dict[str, Path]:
    ...
    for item in bundle["files"]:
        relative = item["path"]
        if FORBIDDEN.intersection(Path(relative).parts) or Path(relative).suffix not in {".py", ".yml", ".yaml", ".toml", ".txt", ".lock"}:
        ...
        if canonical_digest(path.read_bytes()) != item["digest"]:
            raise IntegrityError(f"Source Bundle drift：{relative}")
...
def binding(repository: Path, study_id: str, authority: Path, package: Path = PACKAGE) -> dict:
    ...
    settings = {}
    for name in ("source-bundle.yml", "preregistration.yml", "qualification-spec.yml",
                 "candidate-definition.yml", "development-trial-inputs.yml",
                 "implementation-contract.yml", "runner-contract.yml"):
        path = research / name
    ...
    return {"study_id": study_id, "repository": str(repository.resolve()),
            "authority_root": str(authority.resolve()), "settings": settings,
            "source_bundle": bundle, "environment": environment_identity(),
            "workflow_digest": build_release_manifest(package)["workflow_digest"]}
...
def launch(repository: Path, package: Path, runner: str, request: Path, output: Path) -> None:
    ...
    env = {"PATH": os.defpath, "PYTHONPATH": os.pathsep.join(map(str, (
        package, repository, repository / "src"))), "PYTHONDONTWRITEBYTECODE": "1",
        "PYTHONHASHSEED": "0", "HOME": str(output.parent), "TMPDIR": str(output.parent)}
    process = subprocess.run(
        [sys.executable, "-m", "operations.launch", runner,
         "--request", str(request), "--output", str(output)],
        cwd=repository, env=env, capture_output=True, text=True, timeout=120,
    )
...
def validate_output(value: dict, stage: str, prereg: dict, inputs: dict,
                    bundle: dict, package: Path) -> dict:
    if stage == "development":
        if set(value) != {"candidate", "baseline"}:
            raise ValidationError("Development 輸出必須同時包含 candidate 與 baseline")
        results = {}
        for name in ("candidate", "baseline"):
            # baseline 的績效也重算，但不進入候選 family 或 trial registry。
            item = value[name]
            model_inputs = dict(inputs)
            model_id = inputs["candidate_id"] if name == "candidate" else prereg["baseline_definition"]["baseline_id"]
            model_inputs["candidate_id"] = model_id
            failures = validate_development_trial(
                item, model_inputs, prereg, trial_id=model_id,
            ...
            results[name] = {"trades": len(item["trades"]), "failed_gates": failures}
        return results
    ...
    _, failures = evaluate_historical(value, prereg["evaluation_gates"],
...
def runner_preflight(repository: Path, study_id: str, authority: Path,
    ...
    if contract.get("protocol") != "request-output-v1" or contract.get("synthetic_only") is not True:
    ...
    for stage in ("development", "historical-evaluation"):
        if not {"trades", "no-trades"}.issubset({c["expect"] for c in cases if c["stage"] == stage}):
    ...
    with tempfile.TemporaryDirectory(prefix="runner-preflight-") as temp:
        ...
        shutil.copytree(package, staged_package, ignore=shutil.ignore_patterns(
        ...
        for index, case in enumerate(cases):
            ...
            request = {"stage": case["stage"], "data_path": str(data_path.relative_to(isolated)),
                       "data_digest": canonical_digest(data), "preregistration": prereg,
                       "trial_inputs": inputs, "source_bundle": before["source_bundle"]}
            request_path = output_root / "request.yml"
            request_path.write_bytes(canonical_bytes(request))
            output = output_root / "evidence.yml"
            launch(isolated, staged_package, contract["runners"][case["stage"]], request_path, output)
            value = load_canonical(output)
            result = validate_output(value, case["stage"], prereg, inputs, before["source_bundle"], staged_package)
            count = result["candidate"]["trades"]
            if (case["expect"] == "no-trades" and count != 0) or (case["expect"] == "trades" and count == 0):
                raise ValidationError("合成案例未觸發要求的交易分支")
            for key in case.get("required_output_keys", []):
                current = value
                for part in key.split("."):
                    current = current[part]
            records.append({"case": index, "stage": case["stage"], "expect": case["expect"],
                            "data_digest": canonical_digest(data), "request_digest": canonical_digest(request),
                            "evidence_digest": canonical_digest(value), "validation": result})
...
def verify_report(report: dict, repository: Path, study_id: str, authority: Path,
                  package: Path = PACKAGE) -> None:
    ...
    if report["binding"] != binding(repository, study_id, authority, package):
        raise IntegrityError("prepare 報告已過期；source、設定、工具或環境改變")
