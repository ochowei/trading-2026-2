...
def source_files(repository: Path, bundle: dict) -> dict[str, Path]:
    ...
    for item in bundle["files"]:
        ...
        if canonical_digest(path.read_bytes()) != item["digest"]:
            raise IntegrityError(f"Source Bundle drift：{relative}", path=path, expected=item["digest"], actual=canonical_digest(path.read_bytes()))
...
def runner_preflight(
    ...
    with tempfile.TemporaryDirectory(prefix="runner-preflight-") as temp:
        ...
        for index, (case, inputs) in enumerate(runs):
            ...
            request = {
                "stage": case["stage"],
                "data_path": str(data_path.relative_to(isolated)),
        ...
        for number, (envelope, inputs) in enumerate(development_outputs):
            consumer_package = isolated / "consumer" / str(number) / package.name
            shutil.copytree(staged_package, consumer_package)
            for name in before["settings"]:
                destination = isolated / "research" / study_id / name
                if not destination.exists():
                    destination.parent.mkdir(parents=True, exist_ok=True)
                    destination.write_bytes((research / name).read_bytes())
            consumer_authority = isolated / "consumer-authority" / str(number)
            service = StudyService(
                consumer_package, consumer_authority, repository_root=isolated, allow_draft=True
            )
            report = {
                "schema_version": 1,
                "status": "passed",
                "prepare_checks": "passed",
                "binding": binding(isolated, study_id, consumer_authority, consumer_package),
                "cases": records,
            }
            ...
            def proof(scope):
                return {
                    ...
                    "basis": "隔離 consumer fixture，無正式授權效力",
                    "approved_at": "2000-01-01T00:00:00Z",
                    "scope": scope,
                    "bindings": {
                        "study_id": study_id,
                        "workflow_version": "v002",
                        "source_bundle_digest": canonical_digest(before["source_bundle"]),
            ...
            plan = {
                ...
                "preregistration": prereg,
                "preregistration_actor": "synthetic-fixture",
                "development_actor": "synthetic-fixture",
                "preregistration_approval": proof("preregistration"),
                "development_authorization": proof("development-only"),
            }
...
def verify_report(
    ...
    if report["binding"] != current:
        raise IntegrityError("prepare 報告已過期；source、設定、工具或環境改變", path=repository / "research" / study_id, expected=report["binding"], actual=current)
