...
def create_authorize(service: StudyService, plan: dict, report: dict) -> dict:
    ...
    for proof, actor, scope in (
        (plan["preregistration_approval"], plan["preregistration_actor"], "preregistration"),
        (authorization, plan["development_actor"], "development-only"),
    ):
        approval(
            proof,
            actor,
            scope,
            study_id=study_id,
            source_digest=canonical_digest(report["binding"]["source_bundle"]),
    ...
    with StudyLock(root / ".writer.lock"):
        ...
        approval_path, approval_digest = service.publish_artifact(
            study_id, "evidence/preregistration-approval.yml", plan["preregistration_approval"]
        )
        auth_path, auth_digest = service.publish_artifact(
            study_id, "evidence/development-authorization.yml", authorization
        )
        stages = [
            ("study-created", plan["creator"], None),
            (
                "preregistration-approved",
                plan["preregistration_actor"],
                {
