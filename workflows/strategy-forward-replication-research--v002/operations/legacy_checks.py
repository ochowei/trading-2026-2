...
WORKFLOW_NAME = "strategy-forward-replication-research--v002"
...
def _walk_files(root: Path, suffixes: tuple[str, ...]) -> Iterable[Path]:
    ...
    for path in sorted(root.rglob("*")):
        if any(part in {".super-admin", ".project-manager", "historical-evaluation-artifacts"} for part in path.parts):
            continue
...
def _is_forbidden_read_path(path: Path) -> bool:
    ...
    return any(part in {".super-admin", ".project-manager", "historical-evaluation-artifacts"} for part in path.parts)
...
def _registered_values(
    ...
    return {
        ...
        "price_direction_above": _get_path(
            candidate_direction, "close_above_prior_close"
        ),
        "price_direction_below": _get_path(
            candidate_direction, "close_below_prior_close"
        ),
        ...
        "supplemental_bollinger_lookback": _get_path(
            candidate_supplemental, "bollinger_lookback"
        ),
        ...
        "registered_mean_reversion_gap": _get_path(
            registered_mean, "close_vs_sma20_gap_minimum"
        ),
        ...
        "registered_supplemental_enabled": _get_path(
            registered_supplemental, "enabled"
        ),
        "registered_supplemental_atr_lookback": _get_path(
            registered_supplemental, "atr_lookback"
        ),
        ...
        "registered_intrabar_ambiguity": _get_path(
            registered_execution, "intrabar_ambiguity"
        ),
...
def run_contract(context: StudyContext) -> CheckResult:
    ...
    if isinstance(qualification, dict):
        expected_digest = (
            canonical_digest(prereg_path.read_bytes()) if prereg_path else None
        )
    ...
    try:
        ...
        declared_warmup = _int_value(
            values["fold_warmup_sessions"], "fold warmup sessions"
        )
    ...
    if indicator is not None:
        ...
        volume_prior_only = _get_path(
            indicator, "volume_lead.uses_prior_sessions_only", MISSING
        )
    ...
    if engine_path is not None:
        ...
        if module is not None:
            ...
            if strategy_spec is MISSING:
                ...
                cost_name = engine.get("cost_constant", "BASE_COST") if isinstance(engine, dict) else "BASE_COST"
...
def _signal_audit(
    ...
    *,
    ...
    return {
        ...
        "rejection_reasons": {
            item["session"]: item["reason"] for item in rejected
        },
...
def _run_readiness_case(
    ...
    bars = _make_bars(
        required_history + 12,
        vary_prices=any(
            isinstance(columns.get(label), str)
            for label in ("bollinger", "atr", "pullback_low")
        ),
    )
    ...
    if values.get("volume_lookback", MISSING) is not MISSING:
        expected_columns["volume_lead"] = columns.get(
...
def _run_signal_case(
    ...
    if atr_like:
        for variant in range(8):
            ...
            try:
                candidates.append(
                    ("atr-like", _make_atr_like_bars(rows, event_index=event_index, variant=variant))
                )
    ...
    for kind, bars in candidates:
        try:
            ...
            attempts.append({"kind": kind, "error": str(exc), "classification": "strategy-implementation"})
            continue
    ...
    for first_signal in range(first_signal_start, first_signal_start + 20):
        ...
        for accepted_signal in range(accepted_start, accepted_start + 8):
            ...
            try:
                ...
                raw_indexes = {int(index) for index, value in enumerate(raw_mask.tolist()) if bool(value)}
                backtest_result = _call_backtest(module, bars, spec, cost)
            except SyntheticFixtureInvalid as exc:
                attempts.append({
            ...
            is_valid = (
                ...
                and (
                    rejected_signal is None
                    or bars.index[rejected_signal] not in accepted_sessions
                )
            )
            attempts.append({
                "first_signal_seed": first_signal,
...
def run_diagnose(context: StudyContext) -> CheckResult:
    ...
    fixable: list[str] = []
    if not has_events:
        fixable = [context.display_path(path) for path in _walk_files(
            context.research_root,
            (".yml", ".yaml", ".py", ".md", ".json", ".toml", ".txt"),
        )]
        ...
        if isinstance(source_bundle, dict):
            ...
            if source_expected != prereg_digest:
                result.error(
                    ...
                    path=_display_path(context, source_bundle_path or context.research_root / "source-bundle.yml"),
                    expected=prereg_digest,
            ...
            digest_checks.append(
                {
                    ...
                    "path": context.display_path(source_bundle_path or context.research_root / "source-bundle.yml"),
