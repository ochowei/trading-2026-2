...
def main() -> None:
    ...
    def audit(event: str, values: tuple) -> None:
        ...
        if event == "open" and not isinstance(values[0], int):
            ...
            elif not any(inside(path, parent) for parent in (root, runtime, environment, *timezones)):
        ...
        if event in {"os.remove", "os.rename", "os.rmdir", "os.mkdir", "os.link", "os.symlink"}:
            for item in values[:2] if event in {"os.rename", "os.link", "os.symlink"} else values[:1]:
                if isinstance(item, (str, bytes)) and not inside(Path(os.fsdecode(item)).resolve(), output):
