...
def empty_development_result(preregistration: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, rule in rules.items():
        ...
        gates.append({"gate": name, "operator": rule["operator"],
                      "required": rule["value"], "actual": actual,
                      "passed": False if actual is None else compare(
                          actual, rule["operator"], rule["value"], metric=name)})
    failed = [item["gate"] for item in gates if not item["passed"]]
    return {"metrics": {"completed_trades": 0, "traded_years": 0},
            "diagnostics": {"status": "not-estimable", "reason": "no-trades"},
            "accepted_signal_count": 0, "gates": gates,
            "failed_gates": failed, "disposition": "fail",
