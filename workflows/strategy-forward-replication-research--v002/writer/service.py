...
class StudyService:
    ...
    def _verify_approval(self, event_type: str, actor: str, payload: dict, root: Path) -> None:
        ...
        if event_type not in {"preregistration-approved", "development-authorized"}:
            return
        prefix = "approval" if event_type == "preregistration-approved" else "evidence"
        if f"{prefix}_path" not in payload or f"{prefix}_digest" not in payload:
            raise ValidationError("缺少核准依據與內容指紋")
