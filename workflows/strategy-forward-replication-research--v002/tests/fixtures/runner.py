...
def backtest(rows, *, spec, cost):
    ...
    for offset in range(0, len(rows) - 3, 4):
        ...
        trades.append({"signal_session": signal["Date"], "entry_session": entry["Date"],
                       "exit_session": exit_row["Date"], "exit_reason": "fixture-exit",
                       "held_sessions": 1, "trade_id": f"trade-{offset}",
                       "detail": {"executed_entry_price": str(entry_price),
                                  "executed_exit_price": str(exit_price), "fees": str(cost),
                                  "shares": 1, "pnl": str(pnl),
