...
def add_delayed_qualification(bars: pd.DataFrame, qualification_index: int) -> None:
    """建立資格日下跌、隔日反彈但不改變其他執行條件的合成樣本。"""

    bars.iloc[
        qualification_index - 5, bars.columns.get_loc("Volume")
    ] = 2_000_000.0
    bars.iloc[qualification_index, bars.columns.get_loc("Close")] = 95.0
    bars.iloc[qualification_index, bars.columns.get_loc("Open")] = 100.0
    bars.iloc[qualification_index, bars.columns.get_loc("High")] = 100.0
    bars.iloc[qualification_index, bars.columns.get_loc("Low")] = 94.0
    bars.iloc[qualification_index + 1, bars.columns.get_loc("Close")] = 96.0
    bars.iloc[qualification_index + 1, bars.columns.get_loc("Open")] = 100.0
    bars.iloc[qualification_index + 1, bars.columns.get_loc("High")] = 100.0
    bars.iloc[qualification_index + 1, bars.columns.get_loc("Low")] = 95.0


def set_close_bar(bars: pd.DataFrame, index: int, close: float) -> None:
    bars.iloc[index, bars.columns.get_loc("Close")] = close
    bars.iloc[index, bars.columns.get_loc("Open")] = 100.0
    bars.iloc[index, bars.columns.get_loc("High")] = max(100.0, close)
    bars.iloc[index, bars.columns.get_loc("Low")] = min(100.0, close)


def load_research_runner(name: str, filename: str):
    repository_root = Path(__file__).resolve().parents[1]
    runner_path = repository_root / "research" / "tsm-mean-reversion-two-stage-volume-reversal--v011" / filename
...
def test_delayed_confirmation_uses_qualification_state_and_next_open() -> None:
    bars = make_bars(120)
    qualification_index = 40
    add_delayed_qualification(bars, qualification_index)

    result = backtest(bars, cost=BASE_COST)

    assert result.trades
    trade = result.trades[0]
    assert trade.qualification_session == bars.index[qualification_index]
    assert trade.confirmation_type == "delayed"
    assert trade.signal_session == bars.index[qualification_index + 1]
    assert trade.entry_session == bars.index[qualification_index + 2]


def test_qualification_is_valid_through_third_session_but_expires_without_extension() -> None:
    bars = make_bars(140)
    qualification_index = 40
    add_delayed_qualification(bars, qualification_index)
    set_close_bar(bars, qualification_index + 1, 94.0)
    set_close_bar(bars, qualification_index + 2, 93.0)
    set_close_bar(bars, qualification_index + 3, 92.0)
    set_close_bar(bars, qualification_index + 4, 96.0)
...
def test_delayed_qualification_is_used_once_and_does_not_create_duplicate_entry() -> None:
    bars = make_bars(140)
    qualification_index = 40
    add_delayed_qualification(bars, qualification_index)
    set_close_bar(bars, qualification_index + 2, 97.0)
    set_close_bar(bars, qualification_index + 3, 98.0)

    result = backtest(bars, cost=BASE_COST)

    assert len(result.trades) == 1
    assert result.trades[0].qualification_session == bars.index[qualification_index]
    assert result.trades[0].confirmation_type == "delayed"


def test_future_bar_changes_do_not_change_prior_indicator_or_signal() -> None:
    bars = make_bars(100)
    add_signal(bars, 40)
    original = indicators(bars)

    changed = bars.copy()
    changed.iloc[70, changed.columns.get_loc("Close")] = 250.0
    changed.iloc[70, changed.columns.get_loc("High")] = 250.0
    changed_indicators = indicators(changed)

    columns = [
        "sma_20",
        "rsi_2",
        "prior_volume_spike_ratio",
        "qualification_signal",
        "same_day_confirmation_signal",
        "raw_signal",
    ]
    pd.testing.assert_frame_equal(
        original.loc[: bars.index[40], columns],
        changed_indicators.loc[: bars.index[40], columns],
    )
...
def test_historical_runner_signal_matches_candidate_definition() -> None:
    ...
    runner = load_research_runner(
        "historical_runner_two_stage_signal_v011", "run_historical_evaluation.py"
    )
...
def test_development_runner_preserves_source_lines_when_hashing_snapshot_views(
    ...
    runner = load_research_runner(
        "development_runner_two_stage_digest_v011", "run_development.py"
    )
    source_path = (
        repository_root
        / "research"
...
def test_historical_runner_produces_schema_shape_from_synthetic_folds() -> None:
    ...
    if not runner_path.is_file():
        pytest.skip("Historical runner not yet created")
    runner = load_research_runner("historical_runner_two_stage_v011", "run_historical_evaluation.py")
