...
def load_research_runner(name: str, filename: str):
    ...
    runner_path = repository_root / "research" / "tsm-mean-reversion-two-stage-volume-reversal--v005" / filename
...
def test_historical_runner_signal_matches_candidate_definition() -> None:
    ...
    runner = load_research_runner(
        "historical_runner_two_stage_signal_v005", "run_historical_evaluation.py"
    )
...
def test_development_runner_preserves_source_lines_when_hashing_snapshot_views(
    ...
    repository_root = Path(__file__).resolve().parents[1]
    runner = load_research_runner(
        "development_runner_two_stage_digest_v005", "run_development.py"
    )
    source_path = (
        repository_root
        / "research"
...
def test_historical_runner_produces_schema_shape_from_synthetic_folds() -> None:
    ...
    runner = load_research_runner("historical_runner_two_stage_v005", "run_historical_evaluation.py")
