...
def test_volume_lead_and_price_direction_are_prior_only_and_current_volume_is_ignored() -> None:
    ...
    assert bool(original.iloc[43]["close_below_prior_close"])
...
def test_cooldown_starts_at_completed_exit_and_accepts_only_after_five_sessions() -> None:
    ...
    add_signal(bars, 60)
    add_signal(bars, 61)
    bars.iloc[61, bars.columns.get_loc("Close")] = 95.0
    bars.iloc[61, bars.columns.get_loc("Low")] = 94.0
...
def test_historical_runner_produces_schema_shape_from_synthetic_folds() -> None:
    ...
    spec = importlib.util.spec_from_file_location("historical_runner_v003", runner_path)
