...
def load_research_runner(name: str, filename: str):
    ...
    runner_path = repository_root / "research" / "tsm-mean-reversion-two-stage-volume-reversal--v010" / filename
...
def test_cooldown_starts_at_completed_exit_and_accepts_only_after_three_sessions() -> None:
    bars = make_bars(150)
    add_signal(bars, 40)
    add_signal(bars, 54)
    ...
    assert bars.index[53] not in result.accepted_signal_sessions
...
def test_historical_runner_signal_matches_candidate_definition() -> None:
    ...
    runner = load_research_runner(
        "historical_runner_two_stage_signal_v010", "run_historical_evaluation.py"
    )
...
def test_development_runner_preserves_source_lines_when_hashing_snapshot_views(
    ...
    runner = load_research_runner(
        "development_runner_two_stage_digest_v010", "run_development.py"
    )
    source_path = (
        repository_root
        / "research"
...
def test_historical_runner_produces_schema_shape_from_synthetic_folds() -> None:
    ...
    if not runner_path.is_file():
        pytest.skip("Historical runner not yet created")
    runner = load_research_runner("historical_runner_two_stage_v010", "run_historical_evaluation.py")
