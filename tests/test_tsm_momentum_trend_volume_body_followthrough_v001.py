"""TSM v004 量能加權日內實體延續的 look-ahead 與執行測試。"""
...
from trading_2026_2 import tsm_momentum_trend_volume_body_followthrough_v001 as strategy
...
def make_bars(rows: int = 180, signal_index: int = 50) -> pd.DataFrame:
    ...
    # t-5 的高量日有較大的正向日內實體，其餘四日仍是小幅正實體。
    ...
    # 讓訊號日符合價格加速；此日的 OHLCV 不得影響先前五日實體計算。
...
def test_volume_body_followthrough_precedes_price_acceleration() -> None:
    ...
    assert signal["prior_volume_body_return"] >= 0.0
    assert signal["prior_volume_body_advantage"] >= 0.0
    assert bool(signal["volume_body_followthrough_lead"])
    assert bool(signal["raw_signal"])


def test_body_followthrough_does_not_use_current_bar() -> None:
    bars = make_bars()
    changed = bars.copy()
...
def test_baseline_removes_only_body_followthrough_filter() -> None:
    ...
    assert bool(candidate["volume_body_followthrough_lead"])
    assert bool(candidate["raw_signal"])
    assert bool(baseline["volume_body_followthrough_lead"])
