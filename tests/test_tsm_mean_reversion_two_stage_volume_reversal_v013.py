...
from trading_2026_2.tsm_mean_reversion_two_stage_volume_reversal_v013 import (
    BASE_COST,
    BASELINE_SPEC,
    DEFAULT_SPEC,
    STRESS_COST,
...
def load_research_runner(name: str, filename: str):
    ...
    runner_path = repository_root / "research" / "tsm-mean-reversion-two-stage-volume-reversal--v013" / filename
...
def test_backtest_enters_next_open_and_time_exits_after_10_complete_sessions() -> None:
    bars = make_bars()
    add_signal(bars, 40)
    ...
    assert trade.exit_session == bars.index[51]
...
def test_post_exit_event_cannot_confirm_on_event_day_and_enters_next_open() -> None:
    bars = make_bars(100)
    add_signal(bars, 40)
    # The first position exits at session 51.  Session 52 is deliberately both
    # a valid price-confirmation bar and a volume event; it must still only arm
    # the event.  Session 53 is deliberately down, so session 54 is the first
    # permitted confirmation day with RSI(2) still at or below 50.
    bars.iloc[51, bars.columns.get_loc("Close")] = 95.0
    bars.iloc[51, bars.columns.get_loc("High")] = 100.0
    bars.iloc[51, bars.columns.get_loc("Low")] = 94.0
    bars.iloc[52, bars.columns.get_loc("Volume")] = 2_000_000.0
    bars.iloc[52, bars.columns.get_loc("Close")] = 96.0
    bars.iloc[52, bars.columns.get_loc("Open")] = 100.0
    bars.iloc[52, bars.columns.get_loc("High")] = 101.0
    bars.iloc[52, bars.columns.get_loc("Low")] = 95.0
    bars.iloc[53, bars.columns.get_loc("Close")] = 94.0
    bars.iloc[53, bars.columns.get_loc("Open")] = 100.0
    bars.iloc[53, bars.columns.get_loc("High")] = 101.0
    bars.iloc[53, bars.columns.get_loc("Low")] = 93.0
    bars.iloc[54, bars.columns.get_loc("Close")] = 95.0
    bars.iloc[54, bars.columns.get_loc("Open")] = 100.0
    bars.iloc[54, bars.columns.get_loc("High")] = 101.0
    bars.iloc[54, bars.columns.get_loc("Low")] = 94.0

    result = backtest(bars, cost=BASE_COST)

    assert bars.index[52] not in result.accepted_signal_sessions
    assert result.trades[1].signal_session == bars.index[54]
    assert result.trades[1].entry_session == bars.index[55]
    assert result.entry_modes[1] == "early-cooldown-reset"
    assert result.volume_event_sessions[1] == bars.index[52]


def test_overlapping_event_does_not_reset_window_or_change_trigger_event() -> None:
    bars = make_bars(110)
    add_signal(bars, 40)
    bars.iloc[51, bars.columns.get_loc("Close")] = 90.0
    bars.iloc[51, bars.columns.get_loc("High")] = 100.0
    bars.iloc[51, bars.columns.get_loc("Low")] = 89.0
    for index, close in ((52, 92.0), (53, 91.0), (54, 89.0), (55, 90.0)):
        bars.iloc[index, bars.columns.get_loc("Volume")] = (
            2_000_000.0 if index in (52, 53) else 1_000_000.0
        )
        bars.iloc[index, bars.columns.get_loc("Close")] = close
        bars.iloc[index, bars.columns.get_loc("Open")] = 100.0
        bars.iloc[index, bars.columns.get_loc("High")] = 101.0
        bars.iloc[index, bars.columns.get_loc("Low")] = close - 1.0

    result = backtest(bars, cost=BASE_COST)

    assert result.trades[1].signal_session == bars.index[55]
    assert result.entry_modes[1] == "early-cooldown-reset"
    assert result.volume_event_sessions[1] == bars.index[52]


def test_cooldown_expiry_prioritizes_normal_signal_and_clears_waiting_event() -> None:
    bars = make_bars(110)
    add_signal(bars, 40)
    bars.iloc[51, bars.columns.get_loc("Close")] = 95.0
    bars.iloc[51, bars.columns.get_loc("High")] = 100.0
    bars.iloc[51, bars.columns.get_loc("Low")] = 94.0
    bars.iloc[55, bars.columns.get_loc("Volume")] = 2_000_000.0
    bars.iloc[55, bars.columns.get_loc("Close")] = 95.0
    bars.iloc[55, bars.columns.get_loc("Open")] = 100.0
    bars.iloc[55, bars.columns.get_loc("High")] = 101.0
    bars.iloc[55, bars.columns.get_loc("Low")] = 94.0
    bars.iloc[56, bars.columns.get_loc("Close")] = 96.0
    bars.iloc[56, bars.columns.get_loc("Open")] = 100.0
    bars.iloc[56, bars.columns.get_loc("High")] = 101.0
    bars.iloc[56, bars.columns.get_loc("Low")] = 95.0

    result = backtest(bars, cost=BASE_COST)

    assert result.trades[1].signal_session == bars.index[56]
    assert result.entry_modes[1] == "normal-v009"
    assert result.volume_event_sessions[1] is None
...
def test_historical_runner_signal_matches_candidate_definition() -> None:
    ...
    if not runner_path.is_file():
        pytest.skip("Historical runner not yet created")
    runner = load_research_runner(
        "historical_runner_two_stage_signal_v013", "run_historical_evaluation.py"
    )
...
def test_development_runner_preserves_source_lines_when_hashing_snapshot_views(
    ...
    if not runner_path.is_file():
        pytest.skip("Development runner not yet created")
    runner = load_research_runner(
        "development_runner_two_stage_digest_v013", "run_development.py"
    )
    source_path = (
        repository_root
        / "research"
...
def test_historical_runner_produces_schema_shape_from_synthetic_folds() -> None:
    ...
    if not runner_path.is_file():
        pytest.skip("Historical runner not yet created")
    runner = load_research_runner("historical_runner_two_stage_v013", "run_historical_evaluation.py")
...
def test_realized_drawdown_is_explicit_and_mtm_diagnostic_captures_intraday_pressure() -> None:
    bars = make_bars()
    add_signal(bars, 40)
    bars.iloc[41, bars.columns.get_loc("Open")] = 100.0
    bars.iloc[41, bars.columns.get_loc("Close")] = 100.0
    bars.iloc[41, bars.columns.get_loc("High")] = 101.0
    bars.iloc[41, bars.columns.get_loc("Low")] = 96.5

    result = backtest(bars, cost=BASE_COST)
    assert result.trades[0].exit_reason == "time"
    assert result.trades[0].pnl > -100.0

    realized = qualification_metrics(result)["maximum_drawdown"]
    mtm = mark_to_market_drawdown(bars, result, cost=BASE_COST)
