...
from trading_2026_2 import tsm_momentum_trend_volume_lead_v001 as strategy
...
def test_distributed_volume_pressure_precedes_price_acceleration() -> None:
    bars = make_bars()
    ...
    assert signal["prior_volume_concentration"] >= 1.25
    assert signal["prior_up_volume_share"] >= 0.50
    assert bool(signal["raw_signal"])


def test_pressure_columns_do_not_use_current_volume() -> None:
    bars = make_bars()
    changed = bars.copy()
    ...
    for column in (
        "prior_volume_concentration",
        "prior_up_volume_share",
        "raw_signal",
    ):
        assert before[column] == after[column]


def test_baseline_is_same_price_path_without_volume_pressure() -> None:
    bars = make_bars()
    ...
    assert not bool(candidate.iloc[40]["pressure_lead"])
