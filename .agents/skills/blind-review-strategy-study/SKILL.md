...
先檢查目前對話內容與本 task 已取得的資訊。如果已經出現目標 Study 的正式 Evaluation、robustness challenge、replay 或 terminal 結果，包括 pass/fail、交易、指標、年度表現或失敗 gate：
...
- 不得執行涵蓋 Evaluation、quarantine 或 replay 日期的回測；Development 前資料只能作為指標 warmup。
...
- Review 內容仍須遵守本技能的盲檢討限制與報告格式，並明確聲明沒有讀取或使用正式 Evaluation、challenge、replay、terminal 結果。

## 完成標準

報告必須用繁體中文，先說明實際問題與影響，再補必要技術細節。結尾列出本次實際讀取的檔案，並明確聲明沒有讀取或使用目標 Study 的正式 Evaluation、challenge、replay、terminal 結果。
