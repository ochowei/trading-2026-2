---
name: study-development-note-authoring-v003
description: 為 strategy-forward-replication-research--v003 的單一 Study 撰寫 300–600 字 Development 成果卡，使用逐 Trial candidate／baseline 證據並區分 gates、targets、證據有效性與凍結資格。僅超級管理者或 study 開發者使用，不讀正式結果。
---

# v003 Development 成果卡

成果卡是給人閱讀的補充筆記，不是正式 evidence、事件或結果。只整理使用者指定的一個 v003 Study，不自行跨 Study 比較。角色須為 **超級管理者** 或 **study 開發者**，不得自行切換；目前角色不符就停止使用技能。
...
```bash
python .agents/skills/study-development-note-authoring-v003/scripts/check_development.py <study-id-or-path> --repository-root <repo> --role 'study 開發者'
```

超級管理者改傳對應 role；工具不證明呼叫者身分。它只接受 v003 studies 的直接目錄，拒絕 symlink／逃逸。已確認曝光可用 `--outcome-exposed` 或個別曝光旗標。非零退出即停止；零退出只表示範圍可讀，另檢查每個 Trial 的 evidence_validity。

工具透過 v003 validator 重算 publication 綁定的 candidate／baseline，沒有讀取事件、authority 或 projection，不能證明 Trial 已登錄或 candidate 已凍結。不要改用 v003 CLI 的完整 status／validate、writer、runner 或臨時重跑。
...
- Development assignment、provenance、selection，僅作 Development 補充，不反推正式事件完成。
