#!/usr/bin/env python3
"""v002 唯讀 Development 檢查：不讀事件、projection 或正式結果。"""
...
def main():
    ...
    parser.add_argument("study", help="唯一的 v002 Study ID 或直接 Study 目錄")
    ...
    try:
        ...
        if IDENTIFIER.fullmatch(args.study):
            ...
            if supplied.parent != studies or ".." in supplied.parts:
                raise ValueError("只接受 v002 studies 的直接子目錄")
            name = supplied.name
        ...
        # 每次獨立 process 僅載入 v002；不 import v001 的共用 status tool。
