"""TSM v004 五日滯後高量中性事件 Development runner。

runner 只接受 Lifecycle 提供的 request，讀取 request 綁定的固定 OHLCV，
並輸出單一 canonical YAML。Historical Evaluation 分支只供 v004 runner
preflight 的隔離合成案例驗證，不執行正式 Evaluation。
"""
...
WORKFLOW_ROOT = REPOSITORY_ROOT / "workflows" / "strategy-forward-replication-research--v004"
...
from trading_2026_2 import (  # noqa: E402
    tsm_momentum_trend_volume_lagged_neutral_impulse_v001 as engine,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="TSM volume-lagged-neutral-impulse Development runner"
    )
...
def paired_trades(
    ...
    *,
    ...
    base_by_key = {item_key: item for item_key, item in (
        (lifecycle_key(item), item) for item in base.trades
    )}
    stress_by_key = {item_key: item for item_key, item in (
        (lifecycle_key(item), item) for item in stress.trades
    )}
...
def gate_records(
    ...
    return records, [item["gate"] for item in records if not item["passed"]]
...
def development_model(
    ...
    *,
    ...
    if not trades:
        ...
        metrics, diagnostics, actuals = _recompute_development(
            draft, preregistration
        )
        gates, failures = gate_records(preregistration, actuals)
        result = {
...
def historical_output(
    ...
    *,
    ...
    base = engine.backtest(frame, spec=engine.DEFAULT_SPEC, cost=engine.BASE_COST)
    stress = engine.backtest(frame, spec=engine.DEFAULT_SPEC, cost=engine.STRESS_COST)
    ...
    if request["stage"] == "historical-evaluation":
        ...
        value = {
            "baseline": development_model(
                ...
                candidate_id=preregistration["baseline_definition"]["baseline_id"],
                preregistration=preregistration,
                inputs=inputs,
