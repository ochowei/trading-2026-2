...
CANDIDATE_ID = "tsm-mr-price-reversal-time-decay-v007"
BASELINE_ID = "tsm-mean-reversion-two-stage-baseline-v007"
STUDY_ID = "tsm-mean-reversion-two-stage-volume-reversal--v007"
ENGINE_PATH = "src/trading_2026_2/tsm_mean_reversion_two_stage_volume_reversal_v007.py"
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="產生 TSM price-reversal time-decay v007 2014--2018 Development raw evidence"
    )
...
def main() -> int:
    ...
    mechanism_ablation = {
        "full_two_stage": strategy_summary(
            bars, FULL_TWO_STAGE_SPEC, initial_cash=initial_cash
        ),
        "volume_only": strategy_summary(bars, VOLUME_ONLY_SPEC, initial_cash=initial_cash),
        "price_only_candidate": strategy_summary(
            bars, DEFAULT_SPEC, initial_cash=initial_cash
        ),
        "interpretation": "事前固定的描述性消融，不參與 candidate selection；四組使用相同成本、部位與生命週期規則。",
    }
    candidate_mtm = {
