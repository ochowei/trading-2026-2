...
from trading_2026_2.tsm_mean_reversion_selling_pressure_rollover_v001 import (
    BASE_COST,
    BASELINE_SPEC,
    DEFAULT_SPEC,
    STRESS_COST,
...
CANDIDATE_ID = "tsm-mr-selling-pressure-rollover-v002"
BASELINE_ID = "tsm-mr-selling-pressure-baseline-v002"
STUDY_ID = "tsm-mean-reversion-selling-pressure-rollover--v002"
ENGINE_PATH = "src/trading_2026_2/tsm_mean_reversion_selling_pressure_rollover_v001.py"
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="產生 TSM 低檔量價背離補充型均值回歸 v002 2014--2018 Development raw evidence"
    )
...
def finalize_status(
    ...
    """在不可覆寫發布前固定 formal gates 與研究目標的分離狀態。"""

    finalized = finalize_development_evidence(evidence, preregistration)
    validate_development_status_table(finalized, preregistration)
    return finalized
...
def main() -> int:
    ...
    cand_v009_origin_dates = set(
        t.signal_session for t in base.trades if t.signal_origin == "path_a"
    )
    cand_supp_origin_dates = set(
        t.signal_session for t in base.trades if t.signal_origin == "path_b"
    )

    displaced_v009_dates = sorted(v009_signal_dates - cand_signal_dates)
    retained_v009_trades = [t for t in base.trades if t.signal_origin == "path_a"]
    supp_trades = [t for t in base.trades if t.signal_origin == "path_b"]
    supp_stress_trades = [t for t in stress.trades if t.signal_origin == "path_b"]
    ...
    mechanism_ablation = {
        ...
        "interpretation": "事前固定之消融比較：移除 Path B 的 SVB 條件後重新計算同一段 Development，僅用來描述新增條件對交易數、報酬與回撤的影響，不改寫候選 gate。",
    }
    ...
    record_target(
        ...
        research_targets["minimum_completed_trades"],
    )
    record_target(
        "minimum_stress_return",
        stress_metrics["return"],
        research_targets["minimum_stress_return"],
    )
    record_target(
        "minimum_stress_profit_factor",
        stress_metrics["profit_factor"],
        research_targets["minimum_stress_profit_factor"],
    )
    record_target(
        "maximum_stress_drawdown",
        stress_metrics["maximum_drawdown"],
        research_targets["maximum_stress_drawdown"],
    )

    fixed_years_rule = research_targets["evaluation_years"]
    fixed_years = fixed_years_rule["value"]
    fixed_years_pass = compare(
        fixed_years,
        fixed_years_rule["operator"],
        fixed_years_rule["value"],
    )
    target_records.append(
        {
            "target": "evaluation_years",
            "actual": fixed_years,
            "operator": fixed_years_rule["operator"],
            "required": fixed_years_rule["value"],
            "passed": fixed_years_pass,
        }
    )
    if not fixed_years_pass:
        target_failures.append("evaluation_years")
    ...
    evidence = {
        ...
        "disposition": "fail" if (workflow_failures or target_failures) else "pass",
    ...
    evidence = finalize_status(evidence, preregistration)
