"""產生 TSM v024 唯一一次 Development evidence。
...
from trading_2026_2 import tsm_mean_reversion_two_stage_volume_reversal_v024 as v024
from trading_2026_2.tsm_mean_reversion_two_stage_volume_reversal_v024 import (
    BASE_COST,
    BASELINE_SPEC,
    DEFAULT_SPEC,
    PRICE_ONLY_SPEC,
...
CANDIDATE_ID = "tsm-mr-two-stage-volume-reversal-uptrend-breakout-v024"
BASELINE_ID = "tsm-mean-reversion-two-stage-baseline-v009"
V009_ID = "tsm-mr-two-stage-calibrated-volume-v009"
STUDY_ID = "tsm-mean-reversion-two-stage-volume-reversal--v024"
ENGINE_PATH = "src/trading_2026_2/tsm_mean_reversion_two_stage_volume_reversal_v024.py"
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="產生 TSM v024 Development raw evidence")
...
def trade_comparison(
    ...
    return {
        ...
        "net_new_minus_displaced": {
            "base": text(added_base_pnl - all_control_only_base_pnl),
            "stress": text(added_stress_pnl - all_control_only_stress_pnl),
            "base_strictly_positive": added_base_pnl - all_control_only_base_pnl > 0,
            "stress_strictly_positive": added_stress_pnl - all_control_only_stress_pnl > 0,
            "formula": "truly added candidate completed PnL - all unpaired v009-only completed PnL",
            "eligibility_definition": "研究目標採保守口徑；保留交易的損益差不放入此目標，所有未配對 v009-only 完成交易均視為被排擠或時點改變並扣除。",
        },
        "portfolio_pnl_reconciliation": {
            "base_candidate_minus_control": text(
                sum(trade.pnl for trade in candidate_base)
                - sum(trade.pnl for trade in control_base)
            ),
            "stress_candidate_minus_control": text(
                sum(trade.pnl for trade in candidate_stress)
                - sum(trade.pnl for trade in control_stress)
            ),
            "base_components": {
                "retained_pnl_delta": text(retained_base_pnl_delta),
                "truly_added_pnl": text(added_base_pnl),
                "less_all_control_only_pnl": text(-all_control_only_base_pnl),
            },
            "stress_components": {
                "retained_pnl_delta": text(retained_stress_pnl_delta),
                "truly_added_pnl": text(added_stress_pnl),
                "less_all_control_only_pnl": text(-all_control_only_stress_pnl),
            },
            "base_residual": text(
                sum(trade.pnl for trade in candidate_base)
                - sum(trade.pnl for trade in control_base)
                - (retained_base_pnl_delta + added_base_pnl - all_control_only_base_pnl)
            ),
            "stress_residual": text(
                sum(trade.pnl for trade in candidate_stress)
                - sum(trade.pnl for trade in control_stress)
                - (retained_stress_pnl_delta + added_stress_pnl - all_control_only_stress_pnl)
            ),
        },
    }
...
def main() -> int:
    ...
    mechanism_ablation = {
        "v024_combined": _metric_summary(v024, bars, DEFAULT_SPEC, initial_cash),
        "v009_original_path_only": _metric_summary(v024, bars, V009_ONLY_SPEC, initial_cash),
        "v024_supplemental_only": _metric_summary(v024, bars, SUPPLEMENTAL_ONLY_SPEC, initial_cash),
        "v024_price_only": _metric_summary(v024, bars, PRICE_ONLY_SPEC, initial_cash),
        "simple_baseline": _metric_summary(v024, bars, BASELINE_SPEC, initial_cash),
        "v009_frozen_control_engine": _metric_summary(v009, bars, v009.DEFAULT_SPEC, initial_cash),
        "interpretation": "事前固定的描述性比較，不參與 candidate selection；補充路徑必須把上升趨勢、放量事件、固定突破價、五日確認、失效與單事件狀態機視為一整組機制，不把結果歸因於其中單一條件。",
    }
    ...
    target_actuals: dict[str, str | int | float] = {
        ...
        "base_maximum_drawdown_not_above_v009": base_metrics["maximum_drawdown"],
        ...
        "net_incremental_base_pnl_positive": float(comparison["net_new_minus_displaced"]["base"]),
        "net_incremental_stress_pnl_positive": float(comparison["net_new_minus_displaced"]["stress"]),
    }
