...
from trading_2026_2.tsm_mean_reversion_supplemental_divergence_v002 import (
    BASE_COST,
    BASELINE_SPEC,
    DEFAULT_SPEC,
    STRESS_COST,
...
CANDIDATE_ID = "tsm-mr-supplemental-divergence-v002"
BASELINE_ID = "tsm-mean-reversion-supplemental-divergence-baseline-v002"
STUDY_ID = "tsm-mean-reversion-supplemental-divergence--v002"
ENGINE_PATH = "src/trading_2026_2/tsm_mean_reversion_supplemental_divergence_v002.py"
...
def leave_one_signal_year_out(
    ...
    for omitted_year in all_years:
        selected_trades = [
            trade for year, items in by_year.items() if year != omitted_year for trade in items
        ]
        rates = trade_rates(tuple(selected_trades), initial_cash)
        metrics = path_metrics(rates, initial_cash)
        result[f"exclude_{omitted_year}"] = {
            "completed_trades": str(len(selected_trades)),
            "return": text(metrics["return"]),
            "profit_factor": text(metrics["profit_factor"]),
            "maximum_drawdown": text(metrics["maximum_drawdown"]),
        }
    return result
...
def main() -> int:
    ...
    mechanism_ablation = {
        "v009_primary_only": {
            "base": v009_base_metrics,
            "stress": v009_stress_metrics,
        },
        "full_candidate": {
            "base": base_metrics,
        ...
        "without_volume_lead": {
            "base": without_vol_base_metrics,
            "stress": without_vol_stress_metrics,
        },
