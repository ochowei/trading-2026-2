...
def _walk_files(root: Path, suffixes: tuple[str, ...]) -> Iterable[Path]:
    ...
    for path in sorted(root.rglob("*")):
        if any(part in {".super-admin", "historical-evaluation-artifacts"} for part in path.parts):
            continue
...
def _is_forbidden_read_path(path: Path) -> bool:
    ...
    return any(part in {".super-admin", "historical-evaluation-artifacts"} for part in path.parts)
