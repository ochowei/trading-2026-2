"""只使用 TSM warmup-only 與 Development 快照產生 v014 candidate evidence。
...
STUDY_ID = "tsm-mean-reversion-two-stage-volume-reversal--v014"
ENGINE_PATH = "src/trading_2026_2/tsm_mean_reversion_two_stage_volume_reversal_v014.py"
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="產生 TSM 放量承接後縮量淺回測 v014 2014--2018 Development raw evidence"
    )
...
def main() -> int:
    ...
    if lifecycle_signatures(v009_only_base) != lifecycle_signatures(v009_base):
        raise RuntimeError("v014 關閉補充路徑後，base 原有路徑未逐筆重現 v009")
    if lifecycle_signatures(v009_only_stress) != lifecycle_signatures(v009_stress):
        raise RuntimeError("v014 關閉補充路徑後，stress 原有路徑未逐筆重現 v009")
    if v009_only_base.accepted_signal_sessions != v009_base.accepted_signal_sessions:
        raise RuntimeError("v014 關閉補充路徑後，base accepted signals 未逐筆重現 v009")
    if v009_only_stress.accepted_signal_sessions != v009_stress.accepted_signal_sessions:
        raise RuntimeError("v014 關閉補充路徑後，stress accepted signals 未逐筆重現 v009")
    ...
    mechanism_ablation = {
        "v014_combined_path": strategy_summary(
        ...
        "interpretation": "事前固定的描述性比較，不參與 candidate selection；v014 與固定 v009 使用同一份 Development 資料、相同成本與執行規則，唯一機制變更是補充的縮量淺回測路徑。",
    }
