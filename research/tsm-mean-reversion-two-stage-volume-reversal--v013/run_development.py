"""只使用 TSM warmup-only 與 Development 快照產生 v013 candidate evidence。
...
from trading_2026_2 import (
    tsm_mean_reversion_two_stage_volume_reversal_v013 as candidate_engine,
)
...
CANDIDATE_ID = "tsm-mr-two-stage-cooldown-reset-v013"
BASELINE_ID = "tsm-mean-reversion-two-stage-baseline-v009"
V009_ID = "tsm-mr-two-stage-calibrated-volume-v009"
STUDY_ID = "tsm-mean-reversion-two-stage-volume-reversal--v013"
ENGINE_PATH = "src/trading_2026_2/tsm_mean_reversion_two_stage_volume_reversal_v013.py"
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="產生 TSM 退場後放量冷卻重設 v013 2014--2018 Development raw evidence"
    )
...
def trade_comparison(
    ...
    return {
        ...
        "net_new_minus_displaced": {
            "base": text(added_base_pnl - displaced_base_pnl),
            "stress": text(added_stress_pnl - displaced_stress_pnl),
            "base_strictly_positive": added_base_pnl - displaced_base_pnl > 0,
            "stress_strictly_positive": added_stress_pnl - displaced_stress_pnl > 0,
        },
    }
...
def main() -> int:
    ...
    mechanism_ablation = {
        "v013_candidate_with_cooldown_reset": engine_summary(
        ...
        "interpretation": "事前固定的描述性比較，不參與 candidate selection；候選與 v009 使用同一份 Development 資料、相同成本與執行規則，差異只限退場後提前冷卻重設。",
    }
    ...
    if base.entry_modes != stress.entry_modes:
        raise RuntimeError("candidate base 與 stress 的 entry mode 不一致")
    if base.volume_event_sessions != stress.volume_event_sessions:
        raise RuntimeError("candidate base 與 stress 的 volume event 不一致")
    entry_mode_counts = dict(
        sorted(
            {
                mode: base.entry_modes.count(mode)
                for mode in set(base.entry_modes)
            }.items()
        )
    )
    ...
    evidence = {
        ...
        "trades": [
            trade_record(
                ...
                base_rate,
                stress_rate,
                base.entry_modes[index - 1],
                base.volume_event_sessions[index - 1],
            )
