...
from trading_2026_2.tsm_mean_reversion_two_stage_volume_reversal_v009 import (
    DEFAULT_SPEC as V009_DEFAULT_SPEC,
)
from trading_2026_2.tsm_mean_reversion_two_stage_volume_reversal_v009 import (
...
from trading_2026_2.tsm_mean_reversion_two_stage_volume_reversal_v009 import (
    qualification_metrics as v009_qualification_metrics,
)
from trading_2026_2.tsm_mean_reversion_two_stage_volume_reversal_v010 import (
    DEFAULT_SPEC as V010_DEFAULT_SPEC,
)
from trading_2026_2.tsm_mean_reversion_two_stage_volume_reversal_v010 import (
    backtest as v010_backtest,
)
from trading_2026_2.tsm_mean_reversion_two_stage_volume_reversal_v010 import (
    qualification_metrics as v010_qualification_metrics,
)
from trading_2026_2.tsm_mean_reversion_two_stage_volume_reversal_v011 import (
    BASE_COST,
    BASELINE_SPEC,
    DEFAULT_SPEC,
    PRICE_ONLY_SPEC,
...
CANDIDATE_ID = "tsm-mr-two-stage-calibrated-volume-v011"
BASELINE_ID = "tsm-mean-reversion-two-stage-baseline-v011"
STUDY_ID = "tsm-mean-reversion-two-stage-volume-reversal--v011"
ENGINE_PATH = "src/trading_2026_2/tsm_mean_reversion_two_stage_volume_reversal_v011.py"
V009_ENGINE_PATH = "src/trading_2026_2/tsm_mean_reversion_two_stage_volume_reversal_v009.py"
V010_ENGINE_PATH = "src/trading_2026_2/tsm_mean_reversion_two_stage_volume_reversal_v010.py"
V009_CONTROL_ID = "tsm-mean-reversion-two-stage-volume-reversal-v009-control"
V010_COMPARISON_ID = "tsm-mean-reversion-two-stage-volume-reversal-v010-comparison"
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="產生 TSM price-reversal time-decay v011 2014--2018 Development raw evidence"
    )
...
def trade_record(
    ...
    base_rate: float,
    stress_rate: float,
    v009_signal_frame: pd.DataFrame,
) -> dict[str, object]:
    lifecycle = (
    ...
    v009_same_day_signal = bool(
        v009_signal_frame.loc[base_trade.signal_session, "raw_signal"]
    )
    return {
        ...
        "qualification_session": str(base_trade.qualification_session.date()),
        "confirmation_type": base_trade.confirmation_type,
        "v009_same_day_signal_at_confirmation": v009_same_day_signal,
        "base": {
...
def decimal_text(value: Decimal) -> str:
    """以 Decimal 的完整十進位表示保存比較值，不做報告用四捨五入。"""

    return format(value, "f")


def precise_return(trades: tuple[Any, ...], initial_cash: float) -> Decimal:
    """以逐筆原始 PnL 轉成 Decimal 後計算累積報酬，供嚴格比較使用。"""
...
def compact_trade_record(
    base_trade: Any, stress_trade: Any, *, v009_signal_frame: pd.DataFrame
) -> dict[str, object]:
    ...
    if hasattr(base_trade, "qualification_session"):
        result.update(
            {
                "qualification_session": str(
                    base_trade.qualification_session.date()
                ),
                "confirmation_type": base_trade.confirmation_type,
                "v009_same_day_signal_at_confirmation": bool(
                    v009_signal_frame.loc[base_trade.signal_session, "raw_signal"]
                ),
            }
        )
...
def trade_comparison(
    ...
    *,
    ...
    """按完整交易生命週期分開保留、增加與被排擠的交易。"""

    candidate_stress_by_key = {
        lifecycle_key(trade): trade for trade in candidate_stress
    }
    ...
    if len(candidate_by_key) != len(candidate_base) or len(v009_by_key) != len(v009_base):
        raise RuntimeError("交易生命週期 key 不唯一，無法建立保留／增加／排擠分類")
    ...
    for trade in candidate_base:
        ...
        if key in v009_by_key:
            retained.append(
                compact_trade_record(
                    trade,
                    candidate_stress_by_key[key],
                    v009_signal_frame=v009_signal_frame,
                )
            )
        else:
            added.append(
    ...
    delayed_added = [
        item
        for item in added
        if item.get("confirmation_type") == "delayed"
        and item.get("v009_same_day_signal_at_confirmation") is False
    ]
    delayed_base_pnl = sum(
        (Decimal(str(item["base_pnl"])) for item in delayed_added), Decimal("0")
    )
    delayed_stress_pnl = sum(
        (Decimal(str(item["stress_pnl"])) for item in delayed_added), Decimal("0")
    )
    return {
        ...
        "added": added,
        "displaced": displaced,
        "delayed_added_not_v009_same_day": {
            "count": len(delayed_added),
            "base_aggregate_pnl": decimal_text(delayed_base_pnl),
            "stress_aggregate_pnl": decimal_text(delayed_stress_pnl),
            "post_cost_positive_in_both_models": (
                delayed_base_pnl > 0 and delayed_stress_pnl > 0
            ),
            "definition": "候選確認型態為 delayed、且確認日原 v009 raw_signal 為 false，按候選完整交易生命週期判定為新增。",
        },
    }
...
def validate_inputs(
    ...
    for reference_path in (V009_ENGINE_PATH, V010_ENGINE_PATH):
        _assert_equal(
            f"reference execution engine {reference_path}",
            digest(_repository_path(reference_path)),
...
def main() -> int:
    ...
    v009_control_base = v009_backtest(
        bars,
        spec=V009_DEFAULT_SPEC,
        cost=BASE_COST,
        signal_start=run_args["signal_start"],
        signal_end=run_args["signal_end"],
    )
    v009_control_stress = v009_backtest(
        bars,
        spec=V009_DEFAULT_SPEC,
        cost=STRESS_COST,
        signal_start=run_args["signal_start"],
    ...
    v010_comparison_base = v010_backtest(
        bars,
        spec=V010_DEFAULT_SPEC,
        cost=BASE_COST,
        signal_start=run_args["signal_start"],
        signal_end=run_args["signal_end"],
    )
    v010_comparison_stress = v010_backtest(
        bars,
        spec=V010_DEFAULT_SPEC,
        cost=STRESS_COST,
        signal_start=run_args["signal_start"],
    ...
    v009_control_base_metrics = v009_qualification_metrics(
        v009_control_base, initial_cash=initial_cash
    )
    v009_control_stress_metrics = v009_qualification_metrics(
        v009_control_stress, initial_cash=initial_cash
    )
    v010_comparison_base_metrics = v010_qualification_metrics(
        v010_comparison_base, initial_cash=initial_cash
    )
    v010_comparison_stress_metrics = v010_qualification_metrics(
        v010_comparison_stress, initial_cash=initial_cash
    )
    v009_signal_frame = v009_indicators(bars, V009_DEFAULT_SPEC)
    mechanism_ablation = {
        "two_stage_without_volume": strategy_summary(
            bars, PRICE_ONLY_SPEC, initial_cash=initial_cash
        ),
        "two_stage_without_price_direction": strategy_summary(
            bars, VOLUME_ONLY_SPEC, initial_cash=initial_cash
        ),
        "v009_control": {
            "base": {
                key: text(value) if isinstance(value, float) else value
                for key, value in v009_control_base_metrics.items()
            },
            "stress": {
                key: text(value) if isinstance(value, float) else value
                for key, value in v009_control_stress_metrics.items()
            },
        },
        "interpretation": "事前固定的描述性消融與控制組，不參與 candidate selection；所有組別沿用相同成本、部位與生命週期規則。",
    }
    candidate_mtm = {
    ...
    comparison = trade_comparison(
        base.trades,
        stress.trades,
        v009_control_base.trades,
        v009_control_stress.trades,
        v009_signal_frame=v009_signal_frame,
    )
    target_rules = preregistration["eligibility_rules"]["research_targets"]
    count_rule = target_rules["trade_count_above_v010"]
    return_rule = target_rules["higher_cost_return_above_v009"]
    delayed_rule = target_rules["delayed_added_post_cost_positive"]
    candidate_stress_return_exact = precise_return(stress.trades, initial_cash)
    v009_stress_return_exact = precise_return(v009_control_stress.trades, initial_cash)
    delayed_added = comparison["delayed_added_not_v009_same_day"]
    delayed_base_pnl = Decimal(str(delayed_added["base_aggregate_pnl"]))
    delayed_stress_pnl = Decimal(str(delayed_added["stress_aggregate_pnl"]))
    research_targets = {
        "trade_count_above_v010": {
            "candidate_completed_trades": len(base.trades),
            "reference_study": count_rule["reference_study"],
            "reference_completed_trades": count_rule["reference_completed_trades"],
            "operator": count_rule["operator"],
            "passed": compare(
                len(base.trades),
                count_rule["operator"],
                count_rule["reference_completed_trades"],
            ),
        },
        "higher_cost_return_above_v009": {
            "candidate_stress_cumulative_return_unrounded": decimal_text(
                candidate_stress_return_exact
            ),
            "v009_stress_cumulative_return_unrounded": decimal_text(
                v009_stress_return_exact
            ),
            "difference_unrounded": decimal_text(
                candidate_stress_return_exact - v009_stress_return_exact
            ),
            "reference_study": return_rule["reference_study"],
            "cost_model": return_rule["cost_model"],
            "operator": return_rule["operator"],
            "strict_unrounded_comparison": True,
            "passed": candidate_stress_return_exact > v009_stress_return_exact,
        },
        "delayed_added_post_cost_positive": {
            "scope": delayed_rule["scope"],
            "count": delayed_added["count"],
            "base_aggregate_pnl_unrounded": decimal_text(delayed_base_pnl),
            "stress_aggregate_pnl_unrounded": decimal_text(delayed_stress_pnl),
            "operator": delayed_rule["operator"],
            "require_both_cost_models": delayed_rule["require_both_cost_models"],
            "passed": delayed_base_pnl > 0 and delayed_stress_pnl > 0,
        },
    }
    research_target_failures = [
        name for name, record in research_targets.items() if not record["passed"]
    ]
    candidate_selection_eligible = not failures and not research_target_failures
    ...
    evidence = {
        ...
        "reference_controls": {
            ...
            "v010_comparison": {
                "comparison_id": V010_COMPARISON_ID,
                "development_completed_trades": len(v010_comparison_base.trades),
                "base": {
                    key: text(value) if isinstance(value, float) else value
                    for key, value in v010_comparison_base_metrics.items()
                },
                "stress": {
                    key: text(value) if isinstance(value, float) else value
                    for key, value in v010_comparison_stress_metrics.items()
                },
            },
        },
        "research_targets": research_targets,
        "candidate_selection_eligible": candidate_selection_eligible,
        "candidate_selection_ineligibility_reasons": research_target_failures,
