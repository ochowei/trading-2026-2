...
from trading_2026_2 import (
    tsm_mean_reversion_bollinger_rebound_v001 as candidate_engine,
)
from trading_2026_2 import (
...
CANDIDATE_ID = "tsm-mr-bollinger-rebound-v001"
BASELINE_ID = "tsm-mr-bollinger-simple-baseline-v001"
...
ENGINE_PATH = "src/trading_2026_2/tsm_mean_reversion_bollinger_rebound_v001.py"
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="產生 TSM 布林位置止跌 v001 2014--2018 Development raw evidence"
    )
...
def _source_lines_digest(path: Path, start: str, end: str) -> str:
    ...
    return view_digest(path, start, end)
...
def read_reference_view(path: Path, start: str, end: str) -> pd.DataFrame:
    ...
    return read_development_view(path, start, end)
...
def confirmation_type(signal_session: pd.Timestamp, frame: pd.DataFrame) -> str:
    ...
    if bool(row["raw_signal"]) and bool(row["close_above_prior_close"]):
        return "bollinger_oversold_with_close_rebound"
    raise RuntimeError("交易訊號未通過布林跌深與收盤止跌規則")
...
def trade_comparison(
    ...
    displaced_rows = [
        row(key, v009_control[key], kind="v009_only_displaced") for key in displaced_keys
    ]
...
def validate_inputs(
    ...
    _assert_equal(
        "trial inputs study procedure", inputs["study_procedure_digest"], procedure_digest
    )
    ...
    candidate = load_canonical(args.trial_inputs.parent / "candidate-definition.yml")
    if candidate["signal"] != signal_definition() or inputs["signal"] != signal_definition():
        raise RuntimeError("候選、Development inputs 與實作的完整訊號定義不一致")
    if (
        preregistration["eligibility_rules"]["accepted_signal"]["bollinger"]
        != signal_definition()["bollinger"]
    ):
        raise RuntimeError("Preregistration 的布林設定與實作不一致")
    ...
    _assert_equal(
        "warmup data 與 acquisition manifest", warmup_digest, roles["warmup-only"]["data_digest"]
    )
    _assert_equal(
        "Development data 與 acquisition manifest",
        development_digest,
    ...
    _assert_equal(
        "acquisition manifest 與 Source Bundle", acquisition_digest, entries[ACQUISITION_PATH]
    )
...
def main() -> int:
    ...
    snapshot_set = load_canonical(args.trial_inputs.parent / "data-snapshot-set.yml")
    for role, frame in (("warmup-only", warmup), ("development", development)):
        snapshot = next(item for item in snapshot_set["snapshots"] if item["role"] == role)
        if list(frame.index.strftime("%Y-%m-%d")) != snapshot["sessions"]:
            raise RuntimeError(f"{role} session inventory 與 frozen snapshot set 不一致")
    ...
    mechanism_ablation = {
        "bollinger_rebound_candidate": engine_summary(
        ...
        "simple_baseline": engine_summary(
            candidate_engine, bars, candidate_engine.BASELINE_SPEC, initial_cash=initial_cash
        ),
        "interpretation": "事前固定的描述性比較，不參與 candidate selection；v009、v010 與候選使用同一份 Development 資料，候選只替換跌深偵測，交易配對區分保留、新增與被排擠生命週期。",
    }
    ...
    session_base_rates = _session_realized_rates(bars, base.trades, initial_cash=initial_cash)
    session_stress_rates = _session_realized_rates(bars, stress.trades, initial_cash=initial_cash)
    ...
    confirmation_counts = {
        "bollinger_oversold_with_close_rebound": 0,
    }
    ...
    evidence = {
        ...
        "baseline_comparison": {
            ...
            "candidate_minus_baseline": {
                ...
                "stress_return": text(
                    float(stress_metrics["return"]) - float(baseline_stress_metrics["return"])
                ),
            },
        },
        "bindings": bindings,
        ...
        "candidate_selection": {
            ...
            "requirements": preregistration["eligibility_rules"]["research_targets"][
                "candidate_selection"
            ],
        },
        "risk_diagnostics": {
