"""產生 frozen TSM Bollinger rebound v001 Historical Evaluation raw evidence。
...
from trading_2026_2.frozen_ohlcv_views_v001 import read_view, view_digest
...
def validate_frozen_inputs(args: argparse.Namespace) -> dict:
    ...
    if (
        ...
        != preregistration["eligibility_rules"]["accepted_signal"]["symbol"]
    ):
    ...
    if (
        candidate["execution"]["held_complete_sessions"] + 1
        != preregistration["maximum_holding_sessions"]
    ):
    ...
    if (
        candidate["signal"]["symbol"]
        != preregistration["selection_rule"]["selected_candidate_id"].split("-", 1)[0].upper()
    ):
    ...
    acquisition = load_canonical(
        REPOSITORY_ROOT / "research" / STUDY_ID / "data-snapshot-acquisition.yml"
    )
    role = acquisition["roles"]["historical-evaluation"]
    shared_path = (REPOSITORY_ROOT / role["source_path"]).resolve()
    if args.evaluation.resolve() != shared_path:
        raise RuntimeError("Evaluation 必須直接引用預先綁定的公用 immutable source")
    if digest(args.evaluation) != role["source_digest"]:
        raise RuntimeError("Evaluation shared source digest 不一致")
    if (
        view_digest(args.evaluation, "2020-01-01", "2024-12-31")
        != evaluation_snapshot["data_digest"]
    ):
        raise RuntimeError("Evaluation view digest 與 frozen snapshot set 不一致")
    supplied = {
        "candidate-definition.yml": args.candidate_definition,
        "preregistration.yml": args.preregistration,
        "qualification-spec.yml": args.qualification_spec,
        "data-snapshot-set.yml": args.snapshot_set,
    }
    entries = {item["path"]: item["digest"] for item in source_bundle["files"]}
    for name, path in supplied.items():
        if digest(path) != entries[f"research/{STUDY_ID}/{name}"]:
            raise RuntimeError(f"CLI input {name} 與 Source Bundle binding 不一致")
    if digest(args.source_bundle) != digest(
        REPOSITORY_ROOT / "research" / STUDY_ID / "source-bundle.yml"
    ):
        raise RuntimeError("Source Bundle identity 不一致")
...
def main() -> int:
    ...
    bars = read_view(args.evaluation, "2020-01-01", "2024-12-31")
    snapshot_set = load_canonical(args.snapshot_set)
    inventory = next(
        item["sessions"]
    ...
    if list(bars.index.strftime("%Y-%m-%d")) != inventory:
        raise RuntimeError("Evaluation session inventory 不一致")
